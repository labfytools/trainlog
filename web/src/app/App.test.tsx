...
describe('shell Trainlog', () => {
  ...
  it('persists English and applies centralized Dashboard navigation labels after reload', async () => {
    mockHealth()
    ...
    expect(screen.queryByText('Prochaine séance')).not.toBeInTheDocument()
    first.unmount()
    window.history.replaceState(null, '', '/')
    render(<App />)
    expect(await screen.findByRole('button', { name: 'Edit layout' })).toBeInTheDocument()
    expect(document.documentElement.lang).toBe('en')
    window.localStorage.removeItem('trainlog.web.language.v1')
  })
})
