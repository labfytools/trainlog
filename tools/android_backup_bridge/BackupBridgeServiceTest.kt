...
class BackupBridgeServiceTest {
    ...
    @Test
    fun schemaSeventeenWalDraftRoundTripsWithoutMigration() {
        val name = "bridge-${UUID.randomUUID()}.db"
        var repository = TrainlogRepository(context, name)
        val service = AndroidBackupService(context)
        try {
            val exercise =
                (repository.createExercise(
                        NewExerciseProfile("Bridge", RecordingMode.SETS, TrackingMode.REPS, 0)
                        ...
                        exercises =
                            listOf(
                                SessionExerciseDraft(
                                    exercise = exercise,
                                    sets = listOf(SessionSetDraft(reps = 7, weightKg = 20.0)),
                                )
                            ),
            ...
            context.getDatabasePath(name).let {
                android.database.sqlite.SQLiteDatabase.openDatabase(
                    ...
                    .use { db ->
                        ...
                        db.execSQL("UPDATE active_session_draft SET weight_text='21,' WHERE id=1")
                        assertEquals(17, db.version)
                    }
            }
            val bytes = ByteArrayOutputStream()
            assertTrue(service.create(repository, bytes) is AndroidBackupResult.Success)
            System.getenv("TRAINLOG_BRIDGE_BACKUP_OUTPUT")?.let {
                java.io.File(it).writeBytes(bytes.toByteArray())
            }
            ...
            context.getDatabasePath(name).let {
                android.database.sqlite.SQLiteDatabase.openDatabase(
                    ...
                    .use { db -> assertEquals(17, db.version) }
            }
        } finally {
            repository.close()
