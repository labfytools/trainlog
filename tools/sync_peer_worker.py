...
CAPS={"generation-manifest-v1","generation-ack-v1","causal-delete-v1","mobile-history-v4","execution-draft-v1"}

def emit(run_id:str,phase:str,**values:object)->None:
    value={"format":"trainlog-sync-progress","version":1,"run_id":run_id,"phase":phase}|values
    print(json.dumps(value,sort_keys=True,separators=(",",":")),flush=True)

def bounded(path:Path,limit:int=65536)->bytes:
    with path.open("rb") as stream:
        raw=stream.read(limit+1)
    if len(raw)>limit: raise RuntimeError(f"{path.name} exceeds bound")
    return raw
...
def load_peer(root:Path,expected:str)->dict:
    ...
    if set(value)!={"format","version","peer_id","capabilities"} or value["format"]!="trainlog-sync-peer" or type(value["version"]) is not int or value["version"]!=1 or value["peer_id"]!=expected or not isinstance(value["capabilities"],list) or not CAPS.issubset(set(value["capabilities"])):
...
def main()->int:
    ...
    with redirect_stdout(sys.stderr):
        stage,manifest,digest=generation.capture_desktop(args.database,args.owned_root,peer["peer_id"],args.run_id,outgoing_id)
    del stage
    ...
    with closing(generation.connect_database(args.database)) as db:
        inbound_row=db.execute("SELECT result FROM sync_consumed_generations WHERE generation_id=?",(ack["generation_id"],)).fetchone()
        outbound_row=db.execute("SELECT status FROM sync_generations WHERE generation_id=?",(outgoing_id,)).fetchone()
        drafts=[{"draft_id":row[0],"state":row[1],"session_type":json.loads(row[2]).get("session_type","training"),"occurrence_count":len(json.loads(row[2]).get("exercises",[]))} for row in db.execute("SELECT session_id,state,payload_json FROM execution_drafts ORDER BY session_id LIMIT 32")]
    if inbound_row!=("consumed",) or outbound_row!=("acknowledged",): raise RuntimeError("generation evidence did not corroborate completion")
    emit(args.run_id,"peer_consumed",outbound_generation_id=outgoing_id)
    report={"format":"trainlog-sync-worker-report","version":1,"run_id":args.run_id,"producer_peer_id":peer["peer_id"],"consumer_peer_id":desktop_peer,"inbound_generation_id":ack["generation_id"],"outbound_generation_id":outgoing_id,"manifest_sha256":digest,"result":"completed","sessions_reconciled":0,"domains":{name:True for name in ("history-v4","execution-draft-v1","causal-delete-v1","equipment","aliases","profiles","body-zones","feedback")},"drafts":drafts,"ai_midpoint":{"result":"not_configured"},"ai_post_sync":{"result":"not_configured"}}
    print(json.dumps(report,sort_keys=True,separators=(",",":")),flush=True);return 0
