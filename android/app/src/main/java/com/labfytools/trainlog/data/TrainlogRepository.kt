...
class TrainlogRepository(
    ...
    private fun saveSleepDiary(
        draft: SleepDiaryDraft,
        quickPublicationAt: String?,
    ): SaveSleepDiaryResult {
        ...
        return try {
            ...
            if (quickPublicationAt != null) {
                /* WHY: generation may publish a quick fact before day validation.
                 * CONTRACT: the immutable revision, guarded current tip, and its
                 * exact quick-publication marker commit or roll back together. */
                db.execSQL(
                    "INSERT OR IGNORE INTO sleep_diary_publication_state(entry_id," +
                        "validated_revision_id,validated_at,acknowledged_revision_id," +
                        "acknowledged_at) SELECT entry_id,NULL,NULL,NULL,NULL FROM " +
                        "sleep_diary_entries WHERE entry_id=? AND current_revision_id=?",
                    arrayOf(entryId, revisionId),
                )
                val marked = db.compileStatement(
                    "INSERT INTO sleep_diary_quick_publication_state(entry_id," +
                        "publishable_revision_id,published_at) SELECT entry_id,?,? FROM " +
                        "sleep_diary_entries WHERE entry_id=? AND current_revision_id=? " +
                        "ON CONFLICT(entry_id) DO UPDATE SET " +
                        "publishable_revision_id=excluded.publishable_revision_id," +
                        "published_at=excluded.published_at",
                )
                marked.bindString(1, revisionId)
                marked.bindString(2, quickPublicationAt)
                marked.bindString(3, entryId)
                marked.bindString(4, revisionId)
                if (marked.executeInsert() < 0) throw android.database.sqlite.SQLiteException(
                    "Quick Sleep publication tip invariant failed",
                )
            }
    ...
    private fun pendingSleepDiaryEntry(at: String): SleepDiaryEntry? {
        val dates = quickSleepDates(at) ?: return null
        return listSleepDiary().firstOrNull { entry ->
            entry.nightStartDate == dates.first &&
                entry.events.none { it.type == SleepEventType.FINAL_GET_UP }
        }
    }
    ...
    fun quickSleepBedTime(at: String = OffsetDateTime.now().toString()): SleepQuickActionResult {
        ...
        val pending = pendingSleepDiaryEntry(at)
        if (pending != null) {
            return mutateSleepEntry(pending, at) { current ->
                current.quickDraft(
                    at,
                    events = current.events + SleepDiaryEvent(
                        sleepId("sle"),
                        SleepEventType.BED_TIME,
                        at,
                        null,
                    ),
                )
    ...
    private fun mutateActiveSleep(
        ...
        return mutateSleepEntry(current, at, transform)
    }

    private fun mutateSleepEntry(
        current: SleepDiaryEntry,
        at: String,
        transform: (SleepDiaryEntry) -> SleepDiaryDraft,
    ): SleepQuickActionResult {
        if (runCatching { OffsetDateTime.parse(at) }.isFailure) return SleepQuickActionResult.Error
        return when (val result = saveSleepDiary(transform(current), quickPublicationAt = at)) {
            is SaveSleepDiaryResult.Saved ->
                SleepQuickActionResult.Applied(
    ...
    fun quickSleepMedication(
        ...
        if (quantity !in 1..99) return SleepQuickActionResult.Error
        val medication =
        ...
        val dates = quickSleepDates(at) ?: return SleepQuickActionResult.Error
        val current = pendingSleepDiaryEntry(at)
        val result = if (current == null) {
            saveSleepDiary(
                SleepDiaryDraft(
                    ...
                    intakes = listOf(
                        MedicationIntake(
                            sleepId("mdi"), medication.medicationId, medication.name, at,
                            medication.defaultDoseValue, medication.defaultDoseUnit, "", at, quantity,
                        ),
                    ),
                ),
                quickPublicationAt = at,
                ...
                when (saved) {
                    is SaveSleepDiaryResult.Saved -> SleepQuickActionResult.Applied(
                        SleepQuickActionReceipt(saved.entryId, null, saved.revisionId),
                    )
                    SaveSleepDiaryResult.Conflict -> SleepQuickActionResult.Conflict
                    else -> SleepQuickActionResult.Error
                }
            }
        } else mutateSleepEntry(current, at) { currentEntry ->
            currentEntry.quickDraft(
                at,
                intakes = currentEntry.intakes + MedicationIntake(
    ...
    private fun loadSleepDiaryRevision(entryId: String, revisionId: String): SleepDiaryEntry? {
        ...
        val intakes = db.rawQuery(
            "SELECT intake_id,medication_id,medication_name,taken_at,dose_value,dose_unit,quantity,note,created_at " +
                "FROM sleep_medication_intakes WHERE revision_id=? ORDER BY taken_at,intake_id",
            arrayOf(revisionId),
        ).use { c -> buildList {
            while (c.moveToNext()) add(MedicationIntake(c.getString(0), c.getString(1), c.getString(2), c.getString(3), if (c.isNull(4)) null else c.getDouble(4), if (c.isNull(5)) null else c.getString(5), c.getString(7), c.getString(8), c.getInt(6)))
        } }
    ...
    fun undoSleepQuickAction(
        ...
        return when (val result = saveSleepDiary(compensating, quickPublicationAt = at)) {
            is SaveSleepDiaryResult.Saved ->
                SleepQuickActionResult.Applied(
                    SleepQuickActionReceipt(current.entryId, current.revisionId, result.revisionId),
                )
        ...
        return db.rawQuery("SELECT e.entry_id,e.night_start_date,e.night_end_date,e.created_at," +
            ...
            buildList {
                while (cursor.moveToNext()) {
                    ...
                    val intakes = db.rawQuery("SELECT intake_id,medication_id,medication_name,taken_at,dose_value,dose_unit,quantity,note,created_at FROM sleep_medication_intakes WHERE revision_id=? ORDER BY taken_at,intake_id", arrayOf(revision)).use { intakeCursor -> buildList { while (intakeCursor.moveToNext()) add(MedicationIntake(intakeCursor.getString(0), intakeCursor.getString(1), intakeCursor.getString(2), intakeCursor.getString(3), if (intakeCursor.isNull(4)) null else intakeCursor.getDouble(4), if (intakeCursor.isNull(5)) null else intakeCursor.getString(5), intakeCursor.getString(7), intakeCursor.getString(8), intakeCursor.getInt(6))) } }
    ...
    internal fun buildSleepDiaryV1Json(): String {
        ...
        db.rawQuery(
            "SELECT e.entry_id,e.night_start_date,e.night_end_date,e.created_at,e.updated_at," +
                "e.current_revision_id,e.deleted FROM sleep_diary_entries e LEFT JOIN " +
                "sleep_diary_publication_state p ON p.entry_id=e.entry_id LEFT JOIN " +
                "sleep_diary_quick_publication_state q ON q.entry_id=e.entry_id WHERE " +
                "p.validated_revision_id=e.current_revision_id OR " +
                "q.publishable_revision_id=e.current_revision_id ORDER BY e.entry_id",
            null,
        ).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                db.rawQuery(
                    "SELECT intake_id,medication_id,medication_name,taken_at,dose_value,dose_unit," +
                        "quantity,note,created_at FROM sleep_medication_intakes WHERE revision_id=? ORDER BY intake_id",
                    arrayOf(revisionId),
                    ...
                    .put("quantity", values.getInt(6))
                    .put("note", values.getString(7)).put("created_at", values.getString(8))) }
        ...
        return JSONObject().put("format", "trainlog-sleep-diary").put("version", 2)
    ...
    internal fun applySleepDiaryV1Json(json: String): SleepDiaryImportResult {
        ...
        if (!jsonHasUniqueObjectKeys(json) || root.optString("format") != "trainlog-sleep-diary" ||
            root.optInt("version", 0) !in 1..2 || !root.has("entries") || !root.has("medications"))
            return SleepDiaryImportResult.Rejected("invalid envelope")
        val wireVersion = root.getInt("version")
                ...
                if (local?.first == revisionId) {
                    ...
                    for (intakeIndex in 0 until replayIntakes.length()) {
                        ...
                        sameEvents = sameEvents && db.rawQuery("SELECT medication_id,medication_name," +
                            "taken_at,dose_value,dose_unit,quantity,note,created_at FROM sleep_medication_intakes " +
                            ...
                            current.moveToFirst() && current.getString(0) == intake.getString("medication_id") &&
                                ...
                                current.getInt(5) ==
                                    (if (wireVersion == 1) 1 else intake.getInt("quantity")) &&
                    ...
                    item.getString("treatment_and_notes"), buildList {
                        ...
                        for (intakeIndex in 0 until intakes.length()) {
                            ...
                            val expectedKeys = mutableSetOf(
                                "intake_id", "medication_id", "medication_name", "taken_at",
                                "dose_value", "dose_unit", "note", "created_at",
                            )
                            if (wireVersion == 2) expectedKeys += "quantity"
                            if (intake.keys().asSequence().toSet() != expectedKeys)
                                throw IllegalArgumentException("invalid medication intake fields")
                            ...
                            val quantity = if (wireVersion == 1) 1 else intake.getInt("quantity")
                            add(MedicationIntake(intake.getString("intake_id"), intake.getString("medication_id"),
                                ...
                                intake.getString("note"), intake.getString("created_at"), quantity))
                        }
                    })
    ...
    fun correctCompletedSession(
        ...
        return try {
            ...
            if (!markCausalMutation(db, "session", sessionId))
                return CorrectCompletedSessionResult.DatabaseError("The session is causally deleted.")
            db.setTransactionSuccessful()
    ...
    private fun causalSnapshot(db: SQLiteDatabase, kind: String, targetId: String): JSONArray? {
        if (kind == "session") return causalSessionSnapshot(db, targetId)
        val query = when (kind) {
            "execution_draft" -> Pair("SELECT session_id,revision_id FROM execution_draft_candidates WHERE session_id=? UNION ALL SELECT session_id,revision_id FROM active_session_draft WHERE session_id=?", arrayOf(targetId, targetId))
            ...
            "body_observation" -> Pair("SELECT bo.observation_id,bo.observed_at,bo.body_weight_kg,bo.neck_cm,bo.shoulders_cm,bo.chest_cm,bo.waist_cm,bo.hips_cm,bo.left_arm_cm,bo.right_arm_cm,bo.left_forearm_cm,bo.right_forearm_cm,bo.left_thigh_cm,bo.right_thigh_cm,bo.left_calf_cm,bo.right_calf_cm,bo.notes,s.session_id,(SELECT revision_id FROM sync_note_state WHERE owner_kind='observation' AND owner_id=bo.observation_id) FROM body_observations bo LEFT JOIN sessions s ON s.id=bo.session_row_id WHERE bo.observation_id=?", arrayOf(targetId))
        ...
        val root = db.rawQuery(query.first, query.second).use { cursor ->
        ...
        if (kind == "feedback" && root != null) {
            val revisions = JSONArray()
            db.rawQuery("SELECT revision_id,created_at,raw_text FROM exercise_feedback_revisions WHERE feedback_id=? ORDER BY created_at,revision_id", arrayOf(targetId)).use { cursor ->
                while (cursor.moveToNext()) revisions.put(causalCursorRow(cursor))
            }
            root.put(revisions)
        }
        return root
    ...
    private fun causalCursorRow(cursor: Cursor): JSONArray = JSONArray().also { row ->
        repeat(cursor.columnCount) { index -> row.put(if (cursor.isNull(index)) JSONObject.NULL else when (cursor.getType(index)) {
            Cursor.FIELD_TYPE_INTEGER -> cursor.getLong(index)
            Cursor.FIELD_TYPE_FLOAT -> cursor.getDouble(index)
            else -> cursor.getString(index)
        }) }
    ...
    private fun causalSessionSnapshot(db: SQLiteDatabase, sessionId: String): JSONArray? {
        ...
        db.rawQuery("SELECT se.id,se.entry_id,e.exercise_id,se.position,se.recording_mode,se.tracking_mode,se.data_fields,eq.equipment_id,se.load_mode,se.rest_seconds,se.target_sets,se.target_reps,se.target_duration_seconds,se.target_weight_kg,se.notes FROM session_exercises se JOIN exercises e ON e.id=se.exercise_row_id LEFT JOIN equipment eq ON eq.id=se.equipment_row_id WHERE se.session_row_id=? ORDER BY se.position,se.entry_id", arrayOf(header.first.toString())).use { cursor ->
            while (cursor.moveToNext()) {
                val rowId = cursor.getLong(0); val occurrence = causalCursorRow(cursor).also { it.remove(0) }
                val sets = JSONArray()
                db.rawQuery("SELECT position,reps,duration_seconds,weight_kg FROM performed_sets WHERE session_exercise_row_id=? ORDER BY position", arrayOf(rowId.toString())).use { values -> while (values.moveToNext()) sets.put(causalCursorRow(values)) }
                val continuous = db.rawQuery("SELECT duration_seconds,speed_kmh,distance_km FROM continuous_activity WHERE session_exercise_row_id=?", arrayOf(rowId.toString())).use { if (it.moveToFirst()) causalCursorRow(it) else JSONObject.NULL }
                val maximum = db.rawQuery("SELECT max_weight_kg FROM max_results WHERE session_exercise_row_id=?", arrayOf(rowId.toString())).use { if (it.moveToFirst()) it.getDouble(0) else JSONObject.NULL }
                val feedback = JSONArray()
                db.rawQuery("SELECT feedback_id,observed_at,raw_text FROM exercise_feedback WHERE session_exercise_row_id=? ORDER BY observed_at,feedback_id", arrayOf(rowId.toString())).use { roots -> while (roots.moveToNext()) {
                    val item = causalCursorRow(roots); val revisions = JSONArray()
                    db.rawQuery("SELECT revision_id,created_at,raw_text FROM exercise_feedback_revisions WHERE feedback_id=? ORDER BY created_at,revision_id", arrayOf(roots.getString(0))).use { values -> while (values.moveToNext()) revisions.put(causalCursorRow(values)) }
                    item.put(revisions); feedback.put(item)
                } }
                val note = db.rawQuery("SELECT revision_id FROM sync_note_state WHERE owner_kind='occurrence' AND owner_id=?", arrayOf(cursor.getString(1))).use { if (it.moveToFirst()) it.getString(0) else JSONObject.NULL }
                occurrence.put(sets).put(continuous).put(maximum).put(feedback).put(note); occurrences.put(occurrence)
            }
        }
        val sessionNote = db.rawQuery("SELECT revision_id FROM sync_note_state WHERE owner_kind='session' AND owner_id=?", arrayOf(sessionId)).use { if (it.moveToFirst()) it.getString(0) else JSONObject.NULL }
        val links = JSONArray()
        db.rawQuery("SELECT observation_id FROM body_observations WHERE session_row_id=? ORDER BY observation_id", arrayOf(header.first.toString())).use { while (it.moveToNext()) links.put(it.getString(0)) }
        return header.second.put(occurrences).put(sessionNote).put(links)
    }
    ...
    private fun hasCausalProtection(kind: String): Boolean = database.readableDatabase.rawQuery(
        "SELECT 1 FROM sync_causal_state WHERE target_kind=? AND deleted=1 LIMIT 1", arrayOf(kind),
    ).use { it.moveToFirst() }

    /* WHY: a supported edit followed by a byte-identical revert is still an
     * intervening causal mutation. CONTRACT: writers advance this revision in
     * their business transaction. INVARIANT: deleted state is never revived. */
    private fun markCausalMutation(db: SQLiteDatabase, kind: String, targetId: String): Boolean {
        val deleted = db.rawQuery("SELECT deleted FROM sync_causal_state WHERE target_kind=? AND target_id=?", arrayOf(kind, targetId)).use { it.moveToFirst() && it.getInt(0) != 0 }
        if (deleted) return false
        db.insertWithOnConflict("sync_causal_state", null, ContentValues().apply {
            put("target_kind", kind); put("target_id", targetId)
            put("current_revision_id", "mu_${UUID.randomUUID()}"); put("deleted", 0); putNull("operation_id")
        }, SQLiteDatabase.CONFLICT_REPLACE)
        return true
    }

    private fun causalTargetAuthorized(db: SQLiteDatabase, kind: String, targetId: String): Boolean {
        if (kind == "exercise") {
            val canonical = resolveExerciseId(db, targetId)
            if (trainingKnowledge.getExerciseKnowledge(targetId) != null ||
                trainingKnowledge.getExerciseKnowledge(canonical) != null) return false
        }
        if (kind == "custom_equipment" && trainingKnowledge.getEquipmentKnowledge(targetId) != null)
            return false
        return true
    }
    ...
    private fun validateCausalOperation(value: JSONObject): Boolean {
        ...
        if (!value.hasExactKeys(keys) || keys.filter { it != "publication_context" }.any { value.value(it) !is String } ||
            value.getString("target_kind") !in causalKinds ||
    ...
    private fun applyCausalOperation(db: SQLiteDatabase, operation: JSONObject): CausalDeleteResult {
        ...
        if (!causalTargetAuthorized(db, kind, target)) return CausalDeleteResult.Invalid
        ...
        val count = db.rawQuery("SELECT COUNT(*) FROM sync_causal_operations", null).use { it.moveToFirst(); it.getInt(0) }
        if (count >= MAX_CAUSAL_OPERATIONS) return CausalDeleteResult.Invalid
        ...
        if (buildCausalDeletionExportV1Json(db).toByteArray(StandardCharsets.UTF_8).size > MAX_CAUSAL_ARTIFACT_BYTES)
    ...
    fun deleteCausally(kind: String, targetId: String, creatorId: String): CausalDeleteResult {
        ...
        return try {
            db.beginTransaction()
            if (!causalTargetAuthorized(db, kind, targetId)) return CausalDeleteResult.Invalid
            val state = db.rawQuery("SELECT current_revision_id,deleted FROM sync_causal_state WHERE target_kind=? AND target_id=?", arrayOf(kind, targetId)).use {
                if (it.moveToFirst()) Pair(it.getString(0), it.getInt(1) != 0) else null
            }
            if (state?.second == true) return CausalDeleteResult.Unchanged(state.first)
            val predecessor = if (state != null && !state.second) state.first else causalLiveRevision(db, kind, targetId)
            if (predecessor == null) {
    ...
    fun buildCausalDeletionExportV1Json(): String =
        buildCausalDeletionExportV1Json(database.readableDatabase).also {
            check(it.toByteArray(StandardCharsets.UTF_8).size <= MAX_CAUSAL_ARTIFACT_BYTES) {
                "Causal protection set exceeds artifact byte bound."
            }
        }
    ...
    fun applyCausalDeletionExportV1Json(json: String): CausalDeleteResult {
        if (json.toByteArray().size > MAX_CAUSAL_ARTIFACT_BYTES || !hasStrictJsonShape(json)) return CausalDeleteResult.Invalid
    ...
    fun linkBodyObservationToSession(observationId: String, sessionId: String?): Boolean {
        ...
        return try {
            ...
            val changed = db.update("body_observations", ContentValues().apply {
                if (parent == null) putNull("session_row_id") else put("session_row_id", parent)
            }, "observation_id=?", arrayOf(observationId)) == 1
            if (!changed || !markCausalMutation(db, "body_observation", observationId)) return false
    ...
    fun listExerciseFeedback(sessionId: String): List<ExerciseFeedback> = buildList {
        database.readableDatabase.rawQuery(
            "SELECT f.feedback_id,s.session_id,se.entry_id,e.exercise_id,f.observed_at,f.raw_text," +
                ...
                "WHERE s.session_id=? AND NOT EXISTS(SELECT 1 FROM sync_causal_state cs " +
                "WHERE cs.target_kind='feedback' AND cs.target_id=f.feedback_id AND cs.deleted=1);", arrayOf(sessionId),
...
private class TrainlogDatabaseHelper(
    ...
    override fun onUpgrade(
        ...
        if (version < 32 && newVersion >= 32) {
            /* CONTRACT: quantity is a per-intake fact. Existing immutable
             * revisions hydrate to one unit; quick publication is a distinct
             * exact-revision marker and never validates an entire diary day. */
            if (!tableHasColumn(db, "sleep_medication_intakes", "quantity")) {
                db.execSQL(
                    "ALTER TABLE sleep_medication_intakes ADD COLUMN quantity INTEGER NOT NULL " +
                        "DEFAULT 1 CHECK(quantity BETWEEN 1 AND 99)",
                )
            }
    ...
    private fun createSleepQuickPublicationTable(db: SQLiteDatabase) {
        db.execSQL(
            """CREATE TABLE IF NOT EXISTS sleep_diary_quick_publication_state(
                entry_id TEXT PRIMARY KEY REFERENCES sleep_diary_entries(entry_id) ON DELETE RESTRICT,
                publishable_revision_id TEXT NOT NULL, published_at TEXT NOT NULL
            )""".trimIndent(),
        )
    }
