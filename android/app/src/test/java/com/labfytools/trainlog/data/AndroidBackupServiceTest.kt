...
class AndroidBackupServiceTest {
    ...
    @Test fun completeBackupRestoresDraftPeerLedgersAndPreferences() {
        val name = "backup-${UUID.randomUUID()}.db"
        var repository = TrainlogRepository(context, name)
        val service = AndroidBackupService(context)
        try {
            val exercise = (repository.createExercise(NewExerciseProfile("Backup exercise",
                RecordingMode.SETS, TrackingMode.REPS, 0)) as CreateExerciseResult.Created).exercise
            assertEquals(ActiveDraftMutationResult.Saved, repository.saveActiveSessionDraft(ActiveSessionDraft(
                exercises = listOf(SessionExerciseDraft(exercise = exercise,
            ...
            context.getSharedPreferences("trainlog_presentation_settings", Context.MODE_PRIVATE)
            ...
            assertEquals("en", context.getSharedPreferences("trainlog_presentation_settings", Context.MODE_PRIVATE).getString("language", null))
        } finally { repository.close(); context.deleteDatabase(name) }
    }

    @Test fun corruptAndTraversalContainersAreRejectedBeforeRestore() {
        val service = AndroidBackupService(context)
        val output = ByteArrayOutputStream()
        java.util.zip.ZipOutputStream(output).use { zip ->
            zip.putNextEntry(java.util.zip.ZipEntry("../database.sqlite")); zip.write(byteArrayOf(1)); zip.closeEntry()
        }
