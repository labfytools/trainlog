...
static const ListDefinition LISTS[] = {
    {"preparation",
     ...
     "FROM session_preparations p JOIN session_preparation_revisions r ON r.revision_id=p.current_revision_id "
     "LEFT JOIN session_preparation_entries e ON e.revision_id=r.revision_id "
     "WHERE (?1='' OR r.title LIKE '%'||?1||'%' COLLATE NOCASE OR EXISTS(SELECT 1 FROM session_preparation_entries pe JOIN exercises x ON x.exercise_id=pe.exercise_id WHERE pe.revision_id=r.revision_id AND x.name LIKE '%'||?1||'%' COLLATE NOCASE)) "
     "GROUP BY p.preparation_id ORDER BY COALESCE(r.planned_for,'9999-12-31'),p.updated_at,p.preparation_id LIMIT ?2 OFFSET ?3"},
    {"proposal",
     "SELECT d.draft_id,COALESCE(d.title,''),d.planned_for,CASE WHEN d.published_at IS NULL THEN 'local' ELSE 'published' END,COUNT(e.entry_id),d.created_at "
     "FROM ai_session_drafts d LEFT JOIN ai_session_draft_entries e ON e.draft_row_id=d.id "
     "WHERE (?1='' OR d.title LIKE '%'||?1||'%' COLLATE NOCASE OR EXISTS(SELECT 1 FROM ai_session_draft_entries ae JOIN exercises x ON x.id=ae.exercise_row_id WHERE ae.draft_row_id=d.id AND x.name LIKE '%'||?1||'%' COLLATE NOCASE)) "
     "GROUP BY d.id ORDER BY COALESCE(d.planned_for,'9999-12-31'),d.created_at,d.draft_id LIMIT ?2 OFFSET ?3"},
    {"draft",
     "SELECT d.session_id,d.session_type,substr(d.started_at,1,10),d.state,COALESCE(json_array_length(d.payload_json,'$.exercises'),0),COALESCE(d.started_at,'') "
     "FROM execution_drafts d WHERE NOT EXISTS(SELECT 1 FROM execution_draft_finalizations f WHERE f.session_id=d.session_id) "
     "AND NOT EXISTS(SELECT 1 FROM sync_causal_state c WHERE c.target_kind='execution_draft' AND c.target_id=d.session_id AND c.deleted=1) "
     "AND (?1='' OR d.session_type LIKE '%'||?1||'%' COLLATE NOCASE) ORDER BY CASE d.state WHEN 'active' THEN 0 ELSE 1 END,COALESCE(d.started_at,''),d.session_id LIMIT ?2 OFFSET ?3"},
    {"history",
     "SELECT s.session_id,s.session_type,substr(s.started_at,1,10),'completed',COUNT(se.entry_id),s.started_at "
     "FROM sessions s LEFT JOIN session_exercises se ON se.session_row_id=s.id "
     "WHERE (?1='' OR s.session_type LIKE '%'||?1||'%' COLLATE NOCASE OR EXISTS(SELECT 1 FROM session_exercises he JOIN exercises x ON x.id=he.exercise_row_id WHERE he.session_row_id=s.id AND x.name LIKE '%'||?1||'%' COLLATE NOCASE)) "
...
static TrainlogStatus write_document(yyjson_mut_doc *document,
                                     char **output_json,
                                     size_t *output_size) {
...
TrainlogStatus trainlog_web_sessions_list_json(TrainlogDatabase *database,
    ...
    *output_size = 0U;
    ...
    if (sqlite3_prepare_v2(database->connection, definition->sql, -1, &statement, NULL) !=
            SQLITE_OK ||
        sqlite3_bind_text(statement, 1, query->search == NULL ? "" : query->search, -1,
...
static TrainlogStatus detail_for_revision(TrainlogDatabase *database,
    ...
    const char *sql = proposal
                          ? "SELECT d.draft_id,COALESCE(d.title,''),d.planned_for,d.session_type,d.notes,i.payload_sha256,d.draft_id,CASE WHEN d.published_at IS NULL THEN 'local' ELSE 'published' END FROM ai_session_drafts d LEFT JOIN ai_session_draft_imports i ON i.draft_row_id=d.id WHERE d.draft_id=?1"
                          : "SELECT p.preparation_id,r.title,r.planned_for,r.session_type,r.notes,COALESCE(p.source_payload_sha256,''),p.current_revision_id,p.delivery_state FROM session_preparations p JOIN session_preparation_revisions r ON r.revision_id=p.current_revision_id WHERE p.preparation_id=?1";
    const char *entry_sql = proposal
                                ? "SELECT e.entry_id,e.position,x.exercise_id,x.name,e.equipment_id,e.recording_mode,e.tracking_mode,e.load_mode,e.target_sets,e.target_reps,e.target_duration_seconds,e.target_weight_kg,e.rest_seconds,NULL FROM ai_session_draft_entries e JOIN exercises x ON x.id=e.exercise_row_id JOIN ai_session_drafts d ON d.id=e.draft_row_id WHERE d.draft_id=?1 ORDER BY e.position,e.entry_id"
                                : "SELECT e.entry_id,e.position,e.exercise_id,COALESCE(x.name,e.exercise_id),e.equipment_id,e.recording_mode,e.tracking_mode,e.load_mode,e.target_sets,e.target_reps,e.target_duration_seconds,e.target_weight_kg,e.rest_seconds,e.notes FROM session_preparation_entries e LEFT JOIN exercises x ON x.exercise_id=e.exercise_id JOIN session_preparations p ON p.current_revision_id=e.revision_id WHERE p.preparation_id=?1 ORDER BY e.position,e.entry_id";
    sqlite3_stmt *statement = NULL;
    ...
    for (column = 0; column < 8; ++column) {
        static const char *const keys[] = {"identity", "title", "planned_for", "session_type",
                                           "notes", "source_fingerprint", "revision_id", "state"};
    ...
    while ((step = sqlite3_step(statement)) == SQLITE_ROW) {
        static const char *const text_keys[] = {"entry_id", NULL, "exercise_id", "exercise_name",
                                                "equipment_id", "recording_mode", "tracking_mode",
                                                "load_mode"};
        ...
        for (index = 8; index <= 12; ++index) {
            static const char *const number_keys[] = {"target_sets", "target_reps",
                                                      "target_duration_seconds", "target_weight_kg",
                                                      "rest_seconds"};
            if (sqlite3_column_type(statement, index) == SQLITE_NULL) {
                (void)yyjson_mut_obj_add_null(document, entry, number_keys[index - 8]);
            } else if (index == 11) {
                (void)yyjson_mut_obj_add_real(document, entry, number_keys[index - 8],
                                               sqlite3_column_double(statement, index));
            } else {
                (void)yyjson_mut_obj_add_sint(document, entry, number_keys[index - 8],
...
static TrainlogStatus detail_for_history(TrainlogDatabase *database,
    ...
    static const char SESSION_SQL[] =
        "SELECT session_id,session_type,started_at,ended_at,notes FROM sessions WHERE session_id=?1";
    static const char ENTRY_SQL[] =
        "SELECT se.entry_id,se.position,x.exercise_id,x.name,se.equipment_id,se.recording_mode,se.tracking_mode,se.load_mode,se.target_sets,se.target_reps,se.target_duration_seconds,se.target_weight_kg,se.rest_seconds,se.notes,se.max_weight_kg FROM session_exercises se JOIN exercises x ON x.id=se.exercise_row_id JOIN sessions s ON s.id=se.session_row_id WHERE s.session_id=?1 ORDER BY se.position,se.entry_id";
    static const char SET_SQL[] =
        "SELECT ps.position,ps.reps,ps.duration_seconds,ps.weight_kg FROM performed_sets ps JOIN session_exercises se ON se.id=ps.session_exercise_row_id WHERE se.entry_id=?1 ORDER BY ps.position";
    sqlite3_stmt *statement = NULL;
    ...
    while ((step = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        static const char *const text_keys[] = {"entry_id", NULL, "exercise_id", "exercise_name",
                                                "equipment_id", "recording_mode", "tracking_mode",
                                                "load_mode"};
        static const char *const numeric_keys[] = {"target_sets", "target_reps",
                                                   "target_duration_seconds", "target_weight_kg",
                                                   "rest_seconds", "notes", "max_weight_kg"};

        if (entry == NULL || sets == NULL ||
            !yyjson_mut_obj_add_sint(document, entry, "position", sqlite3_column_int64(statement, 1))) {
            (void)sqlite3_finalize(statement);
            return TRAINLOG_STATUS_SYSTEM_ERROR;
        }
        for (column = 0; column < 8; ++column) {
        ...
        for (column = 8; column <= 14; ++column) {
            if (column == 13) {
                (void)add_nullable_text(document, entry, numeric_keys[column - 8], statement, column);
        ...
        if (!yyjson_mut_obj_add_val(document, entry, "performed_sets", sets) ||
            sqlite3_prepare_v2(database->connection, SET_SQL, -1, &sets_statement, NULL) != SQLITE_OK ||
        ...
        while ((set_step = sqlite3_step(sets_statement)) == SQLITE_ROW) {
            ...
            if (set == NULL ||
                !yyjson_mut_obj_add_sint(document, set, "position", sqlite3_column_int64(sets_statement, 0))) {
                (void)sqlite3_finalize(sets_statement);
                (void)sqlite3_finalize(statement);
                return TRAINLOG_STATUS_SYSTEM_ERROR;
            }
...
static TrainlogStatus detail_for_draft(TrainlogDatabase *database,
    ...
    if (sqlite3_prepare_v2(database->connection,
                          "SELECT session_id,session_type,started_at,state,revision_id,payload_json FROM execution_drafts WHERE session_id=?1",
...
static bool json_object_has_only(yyjson_val *object,
...
static bool bind_nullable_text(sqlite3_stmt *statement, int index, yyjson_val *value) {
    ...
    return yyjson_is_str(value) &&
           sqlite3_bind_text(statement, index, yyjson_get_str(value),
                             (int)yyjson_get_len(value), SQLITE_TRANSIENT) == SQLITE_OK;
}
...
static TrainlogStatus insert_preparation_entry(TrainlogDatabase *database,
    ...
    static const char *const ALLOWED[] = {
        "entry_id", "exercise_id", "equipment_id", "load_mode", "rest_seconds",
        "target_sets", "target_reps", "target_duration_seconds", "target_weight_kg", "notes"};
    static const char INSERT_SQL[] =
        "INSERT INTO session_preparation_entries(revision_id,entry_id,position,exercise_id,equipment_id,recording_mode,tracking_mode,data_fields,load_mode,rest_seconds,target_sets,target_reps,target_duration_seconds,target_weight_kg,notes) "
        "SELECT ?1,?2,?3,e.exercise_id,?5,e.recording_mode,e.tracking_mode,e.data_fields,?6,?7,?8,?9,?10,?11,?12 FROM exercises e "
        "WHERE e.exercise_id=?4 AND NOT EXISTS(SELECT 1 FROM sync_causal_state c WHERE c.target_kind='exercise' AND c.target_id=e.exercise_id AND c.deleted=1)";
    sqlite3_stmt *statement = NULL;
    const char *entry_id;
    ...
    if (!yyjson_is_obj(entry) ||
        ...
        !bounded_json_string(yyjson_obj_get(entry, "exercise_id"), TRAINLOG_ID_MAX + 1U,
                             &exercise_id) ||
    ...
    } else if (!yyjson_is_int(value) || yyjson_get_sint(value) < 1 || yyjson_get_sint(value) > 10000 ||
    ...
    } else if (!yyjson_is_int(value) || yyjson_get_sint(value) < 1 || yyjson_get_sint(value) > 604800 ||
...
TrainlogStatus trainlog_web_sessions_save_json(TrainlogDatabase *database,
    ...
    if (database == NULL || expected_revision == NULL || request_id == NULL || request_id[0] == '\0' ||
        save_body == NULL || save_body_size == 0U || output_json == NULL || output_size == NULL ||
        !timestamp_now(now)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    *output_json = NULL;
    *output_size = 0U;
    ...
    if (actual_preparation == NULL || actual_preparation[0] == '\0') {
        if (expected_revision[0] != '\0' ||
            trainlog_id_generate("sp", new_preparation, sizeof(new_preparation)) != TRAINLOG_STATUS_OK) {
            yyjson_doc_free(input);
            return TRAINLOG_STATUS_INVALID_ARGUMENT;
        }
    ...
    if (preparation_id != NULL && preparation_id[0] != '\0') {
        ...
        if (sqlite3_prepare_v2(database->connection,
                              ...
                              -1, &statement, NULL) != SQLITE_OK ||
            sqlite3_bind_text(statement, 1, actual_preparation, -1, SQLITE_TRANSIENT) != SQLITE_OK ||
            sqlite3_step(statement) != SQLITE_ROW) {
        ...
        (void)sqlite3_finalize(statement);
        statement = NULL;
    } else if (sqlite3_prepare_v2(database->connection,
    ...
    if (sqlite3_prepare_v2(database->connection,
        ...
        (expected_revision[0] == '\0' ? sqlite3_bind_null(statement, 3)
    ...
    for (index = 0U; index < yyjson_arr_size(occurrences); ++index) {
        status = insert_preparation_entry(
            database, revision, yyjson_arr_get(occurrences, index), index);
        if (status != TRAINLOG_STATUS_OK) {
            goto rollback;
        }
    }
    if (preparation_id != NULL && preparation_id[0] != '\0') {
        if (sqlite3_prepare_v2(database->connection,
                              "UPDATE session_preparations SET current_revision_id=?1,updated_at=?2,editing_state=?3,delivery_state=CASE WHEN delivery_state='local' THEN 'local' ELSE 'pending' END WHERE preparation_id=?4 AND current_revision_id=?5",
                              -1, &statement, NULL) != SQLITE_OK ||
            sqlite3_bind_text(statement, 1, revision, -1, SQLITE_TRANSIENT) != SQLITE_OK ||
            ...
            sqlite3_bind_text(statement, 4, actual_preparation, -1, SQLITE_TRANSIENT) != SQLITE_OK ||
