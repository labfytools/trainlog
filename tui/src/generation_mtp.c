...
static TrainlogStatus children(const TrainlogGenerationMtpIo *io,
                               const Selection *selection,
                               uint32_t parent,
                               TrainlogMtpEntry *entries,
                               size_t *count) {
    return io->children(selection->bus,
                        selection->device,
                        selection->storage,
                        parent,
                        entries,
                        TRAINLOG_GENERATION_MTP_MAX_CHILDREN,
                        count);
}

static TrainlogStatus one_named(const TrainlogGenerationMtpIo *io,
                                const Selection *selection,
                                uint32_t parent,
                                const char *name,
                                bool folder,
                                TrainlogMtpEntry *result) {
...
static TrainlogStatus select_peer(const TrainlogGenerationMtpIo *io,
                                  const char *expected,
    ...
    if (status != TRAINLOG_STATUS_OK || device_count == 0U) {
        diag(diagnostic, diagnostic_size, "no MTP device available");
        return TRAINLOG_STATUS_NOT_FOUND;
    }
    for (di = 0U; di < device_count; ++di) {
        status = io->storages(devices[di].bus_number,
                              devices[di].device_number,
                              storages,
    ...
    (void)unlink(peer_path);
    if (matches == 0U) {
        diag(diagnostic, diagnostic_size, "expected Android peer not found");
        return TRAINLOG_STATUS_NOT_FOUND;
    }
    if (matches > 1U) {
        diag(diagnostic, diagnostic_size, "ambiguous compatible Android peers");
        return TRAINLOG_STATUS_CONFLICT;
    }
...
static TrainlogStatus pull_folder(const TrainlogGenerationMtpIo *io,
                                  const Selection *selection,
                                  uint32_t parent,
                                  const char *local,
                                  unsigned int depth,
...
static TrainlogStatus push_file(const TrainlogGenerationMtpIo *io,
                                const Selection *selection,
                                uint32_t parent,
                                const char *local,
                                const char *name) {
    TrainlogMtpEntry existing;
    TrainlogStatus status;
    char comparison[1060], temporary_name[256];
    ...
    if (status == TRAINLOG_STATUS_OK) {
        ...
        (void)unlink(comparison);
        (void)snprintf(
            temporary_name, sizeof(temporary_name), ".trainlog-%ld-%s", (long)getpid(), name);
        status = io->send(selection->bus,
                          selection->device,
                          selection->storage,
                          parent,
                          local,
        ...
        if (status != TRAINLOG_STATUS_OK || !same_files(local, comparison)) {
            (void)unlink(comparison);
            (void)io->remove(selection->bus, selection->device, uploaded);
...
static TrainlogStatus push_folder(const TrainlogGenerationMtpIo *io,
                                  const Selection *selection,
                                  uint32_t parent,
                                  const char *local,
                                  unsigned int depth,
...
validate_local_folder(const char *local, unsigned int depth, uint64_t *total, size_t *objects) {
    DIR *directory;
    struct dirent *entry;
    TrainlogStatus status = TRAINLOG_STATUS_OK;
    if (depth > TRAINLOG_GENERATION_MTP_MAX_DEPTH || (directory = opendir(local)) == NULL) {
    ...
    while ((entry = readdir(directory)) != NULL && status == TRAINLOG_STATUS_OK) {
        ...
        if (*objects == TRAINLOG_GENERATION_MTP_MAX_CHILDREN) {
            status = TRAINLOG_STATUS_INVALID_ARGUMENT;
            break;
        }
        ++*objects;
