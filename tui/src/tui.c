...
static const char *dashboard_period_bucket_summary(TrainlogDashboardPeriod period)
{
    switch (period) {
        case TRAINLOG_DASHBOARD_7_DAYS: return "7 jours · quotidien";
        case TRAINLOG_DASHBOARD_30_DAYS: return "30 jours · pas de 5 j";
        case TRAINLOG_DASHBOARD_90_DAYS: return "90 jours · pas de 15 j";
        case TRAINLOG_DASHBOARD_YEAR: return "1 an · pas de 31 j";
        case TRAINLOG_DASHBOARD_ALL: return "Tout · 12 périodes max.";
    }
    return "Période";
}

static void dashboard_period_bucket_label(TrainlogDashboardPeriod period,
    size_t index, size_t count, char output[12])
{
    if (index + 1U == count) {
        (void)snprintf(output, 12U, "%s",
            trainlog_presentation_text("dashboard.current"));
    } else if (period == TRAINLOG_DASHBOARD_7_DAYS) {
        (void)snprintf(output, 12U, "J−%zu", count - 1U - index);
    } else if (period == TRAINLOG_DASHBOARD_YEAR) {
        (void)snprintf(output, 12U, "M−%zu", count - 1U - index);
    } else {
        (void)snprintf(output, 12U, "P−%zu", count - 1U - index);
    }
}

/* The TUI owns only the reference clock and selected presentation period. */
static void __attribute__((unused)) app_shell_load_dashboard(TrainlogAppContext *app)
{
    TrainlogDashboardQuery query;
    TrainlogDashboardPeriod period = app->dashboard.period;
    time_t now = time(NULL);
    query.period = period;
    if (now == (time_t)-1) {
        (void)memset(&app->dashboard, 0, sizeof(app->dashboard));
        app->dashboard.period = period;
        app->dashboard.error = true;
        return;
    }
    query.reference_unix_second = (int64_t)now;
    (void)trainlog_dashboard_load(app->database, &query, &app->dashboard);
}
    ...
    int rows)
{
    ...
    for (index = 0U; index < bucket_rows; ++index) {
        const TrainlogBodyZone *zone = trainlog_body_zone_catalog_lookup(
            dashboard->zones[index].zone_id);
        const char *label = dashboard->zones[index].zone_id[0] == '\0'
            ? trainlog_presentation_text("ui.unclassified")
            : (zone == NULL ? dashboard->zones[index].zone_id : zone->display_name);
        ...
        trainlog_surface_printf(app->content, row, left,
            "%-*.*s", label_width, label_width, label);
        trainlog_surface_set_role(app->content, TRAINLOG_COLOR_GRAPH,
            TRAINLOG_RGB_BASE, TRAINLOG_TEXT_NORMAL);
    ...
    int height)
{
    ...
    for (index = 0U; index < dashboard->week_count; ++index) {
        char label[12];
        dashboard_period_bucket_label(dashboard->period, index,
            dashboard->week_count, label);
        int label_margin = 3;
...
static void __attribute__((unused)) app_shell_render_dashboard(TrainlogAppContext *app)
{
    ...
    trainlog_surface_printf(app->content, 2, 2,
        "Période  %s  %s  %s  %s  %s",
        dashboard->period == TRAINLOG_DASHBOARD_7_DAYS ? "[7j]" : "7j",
        dashboard->period == TRAINLOG_DASHBOARD_30_DAYS ? "[30j]" : "30j",
        dashboard->period == TRAINLOG_DASHBOARD_90_DAYS ? "[90j]" : "90j",
        dashboard->period == TRAINLOG_DASHBOARD_YEAR ? "[1an]" : "1an",
        trainlog_presentation_source(
            dashboard->period == TRAINLOG_DASHBOARD_ALL ? "[Tout]" : "Tout"));
    trainlog_surface_printf(app->content, 3, 2,
