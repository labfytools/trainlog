...
static void sqlite_profile_revision(sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    if (argc != 4 || sqlite3_value_type(argv[0]) != SQLITE_TEXT ||
        ...
        sqlite3_result_error(context, "invalid profile revision", -1);
        return;
    }
    parent = (const char *)sqlite3_value_text(argv[0]);
    ...
    if (length < 0 || (size_t)length >= sizeof(name) ||
        uuid_parse("4f4c8ea6-18f6-5e48-9d65-a54a24889149", namespace_id) != 0) {
        sqlite3_result_error(context, "profile revision overflow", -1);
        return;
    }
...
static void
sqlite_prototype_profile_valid(sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    if ((strcmp(recording, "sets") != 0 && strcmp(recording, "continuous") != 0) ||
        ...
        (strcmp(recording, "continuous") == 0 && strcmp(tracking, "duration") != 0) || fields < 0 ||
        (fields & ~3) != 0 || (strcmp(recording, "sets") == 0 && fields != 0)) {
        sqlite3_result_int(context, 0);
        return;
    }
    length = snprintf(expected, sizeof(expected), "pr1|%s|%s|%d", recording, tracking, fields);
    sqlite3_result_int(
        context, length >= 0 && (size_t)length < sizeof(expected) && strcmp(token, expected) == 0);
}

TrainlogStatus trainlog_database_read_snapshot_begin(TrainlogDatabase *database) {
    char sql[64];
    if (database == NULL || database->connection == NULL ||
        database->read_snapshot_depth == UINT_MAX) {
...
TrainlogStatus trainlog_database_read_snapshot_end(TrainlogDatabase *database,
    ...
    if (commit_snapshot) {
        ...
        (void)snprintf(sql,
                       sizeof(sql),
                       "ROLLBACK TO trainlog_read_%u; RELEASE trainlog_read_%u;",
                       depth,
                       depth);
    }
    if (sqlite3_exec(database->connection, sql, NULL, NULL, NULL) != SQLITE_OK) {
...
TrainlogStatus trainlog_database_scan_generation_history(TrainlogDatabase *database,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        row.equipment_id = sqlite3_column_type(statement, 6) == SQLITE_NULL
                               ? NULL
                               : (const char *)sqlite3_column_text(statement, 6);
        load = sqlite3_column_text(statement, 7);
        if (strcmp((const char *)recording, "sets") == 0) {
            row.recording_mode = TRAINLOG_RECORDING_SETS;
        } else if (strcmp((const char *)recording, "continuous") == 0) {
            row.recording_mode = TRAINLOG_RECORDING_CONTINUOUS;
        } else {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
        if (strcmp((const char *)tracking, "reps") == 0) {
            row.tracking_mode = TRAINLOG_TRACKING_REPS;
        } else if (strcmp((const char *)tracking, "duration") == 0) {
            row.tracking_mode = TRAINLOG_TRACKING_DURATION;
        } else {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
        if (strcmp((const char *)load, "none") == 0) {
            row.load_mode = TRAINLOG_LOAD_NONE;
        } else if (strcmp((const char *)load, "external") == 0) {
            row.load_mode = TRAINLOG_LOAD_EXTERNAL;
        } else if (strcmp((const char *)load, "assistance") == 0) {
            row.load_mode = TRAINLOG_LOAD_ASSISTANCE;
        } else {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
        ...
        if (row.has_explicit_max && ((sqlite3_column_type(statement, 16) != SQLITE_FLOAT &&
        ...
        if (row.has_actual_set) {
            ...
            row.repetitions = sqlite3_column_type(statement, 14) == SQLITE_NULL
                                  ? 0
    ...
    }
    {
        TrainlogStatus end_status =
            trainlog_database_read_snapshot_end(database, status == TRAINLOG_STATUS_OK);
        if (status == TRAINLOG_STATUS_OK) {
            status = end_status;
        }
    }
...
static TrainlogStatus lookup_exercise_row_id(TrainlogDatabase *database,
...
static bool custom_equipment_exists(TrainlogDatabase *database, const char *equipment_id) {
    ...
    int rc;
    bool found = false;
    if (database == NULL || equipment_id == NULL) {
        return false;
    }
    rc = sqlite3_prepare_v2(database->connection,
                            "SELECT 1 FROM custom_equipment WHERE equipment_id=?1;",
                            -1,
                            &statement,
                            NULL);
    if (rc != SQLITE_OK) {
...
static void set_open_diagnostic(
    char *output, size_t capacity, const char *operation, sqlite3 *connection, int sqlite_status) {
    const char *message;
...
static void set_application_diagnostic(char *output, size_t capacity, const char *message) {
    if (output == NULL || capacity == 0U) {
        return;
    }
...
static const char *const MIGRATE_V6_TO_V7_SQL =
    ...
    "INSERT INTO "
    "session_exercises(id,entry_id,session_row_id,exercise_row_id,recording_mode,data_fields,"
    "position,load_mode,rest_seconds,target_sets,target_reps,target_duration_seconds,target_weight_"
    "kg,equipment_id,notes) "
    "SELECT id,'sxe_legacy_' || "
    "id,session_row_id,exercise_row_id,recording_mode,data_fields,position,load_mode,rest_seconds,"
    "target_sets,target_reps,target_duration_seconds,target_weight_kg,equipment_id,notes FROM "
    "session_exercises_v6;"
    ...
    "REFERENCES session_exercises(id) ON DELETE CASCADE,position INTEGER NOT NULL,reps "
    "INTEGER,duration_seconds INTEGER,weight_kg REAL,UNIQUE(session_exercise_row_id,position));"
    "INSERT INTO performed_sets SELECT * FROM performed_sets_v6;"
    "ALTER TABLE continuous_activity RENAME TO continuous_activity_v6;"
    "CREATE TABLE continuous_activity (id INTEGER PRIMARY KEY,session_exercise_row_id INTEGER NOT "
    "NULL UNIQUE REFERENCES session_exercises(id) ON DELETE CASCADE,duration_seconds INTEGER NOT "
    "NULL,speed_kmh REAL,distance_km REAL);"
    "INSERT INTO continuous_activity SELECT * FROM continuous_activity_v6;"
    "DROP TABLE performed_sets_v6;DROP TABLE continuous_activity_v6;DROP TABLE "
    "session_exercises_v6;"
...
static const char *const MIGRATE_V12_TO_V13_SQL_A =
    "BEGIN IMMEDIATE;"
    "ALTER TABLE exercises ADD COLUMN load_semantics TEXT CHECK(load_semantics IN "
    "('none','external','assistance','bodyweight','cardio'));"
    ...
    "UPDATE exercises SET name='Hip Abduction',normalized_name='hip "
    "abduction',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "hip_abduction_v1',science_state='resolved',legacy_equipment_id='hip_abduction' WHERE "
    "exercise_id='ex_7e7cf906-2214-4066-bcb7-c16382d83b3b' AND name='Abduction de hanche assise';"
    "UPDATE exercises SET name='Leg Extension',normalized_name='leg "
    "extension',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "leg_extension_v1',science_state='resolved',legacy_equipment_id='leg_extension' WHERE "
    "exercise_id='ex_1872246a-39ae-44dc-b58d-f87e90ca49ab' AND name='Extension de genou assise';"
    "UPDATE exercises SET name='Back Extension',normalized_name='back "
    "extension',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "back_extension_v1',science_state='resolved',legacy_equipment_id='back_extension' WHERE "
    "exercise_id='ex_474ec393-3efa-4aaa-8e08-1a0245ed7835' AND name='Extension du tronc';"
    "UPDATE exercises SET name='Arm Curl',normalized_name='arm "
    "curl',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_arm_"
    "curl_v1',science_state='resolved',legacy_equipment_id='arm_curl' WHERE "
    "exercise_id='ex_ec619fc2-4685-4044-873c-86764bd4a0fe' AND name='Flexion de coude à la "
    "machine';"
    "UPDATE exercises SET name='Prone Leg Curl',normalized_name='prone leg "
    "curl',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "prone_leg_curl_v1',science_state='resolved',legacy_equipment_id='prone_leg_curl' WHERE "
    "exercise_id='ex_d7398d9f-d928-4d2e-94e9-74e201da55c5' AND name='Flexion de genou couchée';"
    "UPDATE exercises SET name='Rotary Torso',normalized_name='rotary "
    "torso',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "rotary_torso_v1',science_state='resolved',legacy_equipment_id='rotary_torso' WHERE "
    "exercise_id='ex_1a34814c-2e46-40fc-b1f4-6d60b8e5a3e0' AND name='Rotation du tronc à la "
    "machine';";

static const char *const MIGRATE_V12_TO_V13_SQL_B =
    "UPDATE exercises SET name='Seated Row',normalized_name='seated "
    "row',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "seated_row_v1',science_state='resolved',legacy_equipment_id='seated_row' WHERE "
    "exercise_id='ex_33f79331-871c-4eed-babe-346e53a99070' AND name='Tirage horizontal assis';"
    "UPDATE exercises SET name='Diverging Seated Row',normalized_name='diverging seated "
    "row',load_semantics='external',machine_variant='diverging',scientific_profile_id='sp_seated_"
    "row_v1',science_state='resolved',legacy_equipment_id='diverging_seated_row' WHERE "
    "exercise_id='ex_1b0c6b8b-b05e-4e6f-8809-5f7d85d668de' AND name='Tirage horizontal divergent "
    "assis';"
    "UPDATE exercises SET name='Diverging Lat Pulldown',normalized_name='diverging lat "
    "pulldown',load_semantics='external',machine_variant='diverging',scientific_profile_id='sp_"
    "vertical_pull_v1',science_state='resolved',legacy_equipment_id='diverging_lat_pulldown' WHERE "
    "exercise_id='ex_b4d1daf1-de4a-4016-abdf-487bf6014ce6' AND name='Tirage vertical divergent';"
    "UPDATE exercises SET name='Lat Pull',normalized_name='lat "
    "pull',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "vertical_pull_v1',science_state='resolved',legacy_equipment_id='lat_pull' WHERE "
    "exercise_id='ex_a72fa713-4b0e-431d-95e2-42d95beb77b1' AND name='Tirage vertical à la poulie';"
    "UPDATE exercises SET name='Converging Shoulder Press',normalized_name='converging shoulder "
    "press',load_semantics='external',machine_variant='converging',scientific_profile_id='sp_"
    "shoulder_press_v1',science_state='resolved',legacy_equipment_id='converging_shoulder_press' "
    "WHERE exercise_id='ex_6dfc7ffd-8891-464e-a995-808baf1b0d7b' AND name='Développé épaules "
    "convergent';"
    "UPDATE exercises SET name='Seated Leg Curl',normalized_name='seated leg "
    "curl',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_"
    "seated_leg_curl_v1',science_state='resolved',legacy_equipment_id='seated_leg_curl' WHERE "
    "exercise_id='ex_a1ef5047-b44b-4c64-a6ed-c7a3bc13b163' AND name='Flexion de genou assise';"
    "UPDATE exercises SET name='Leg Press',normalized_name='leg "
    "press',load_semantics='external',machine_variant='selectorized',scientific_profile_id='sp_leg_"
    "press_v1',science_state='resolved',legacy_equipment_id='leg_press' WHERE "
    "exercise_id='ex_b432623f-bfe9-4daf-a653-60ec7fdffbde' AND name IN ('Presse à cuisses','Leg "
    "press');"
    "UPDATE exercises SET name='Rear Delt',normalized_name='rear "
    "delt',load_semantics='external',machine_variant='rear_delt',scientific_profile_id='sp_rear_"
    "delt_v1',science_state='resolved',legacy_equipment_id='rear_delt_pec_fly' WHERE "
    "exercise_id='ex_4cd2433e-80b1-478a-b8df-73fc6ef80962' AND name='Écarté inversé à la machine';"
    "INSERT INTO "
    "exercises(exercise_id,name,normalized_name,tracking_mode,recording_mode,data_fields,load_"
    "semantics,machine_variant,scientific_profile_id,science_state,legacy_equipment_id) VALUES"
    "('ex_0e26c06f-a458-40a4-be20-4ed219ede30d','Plate Loaded Leg Press','plate loaded leg "
    "press','reps','sets',0,'external','plate_loaded','sp_leg_press_v1','resolved','plate_loaded_"
    "leg_press'),"
    "('ex_f01d2a46-6984-4dec-8934-4d82fca6dfc2','Treadmill','treadmill','duration','continuous',3,'"
    "cardio','treadmill',NULL,'unresolved','treadmill'),"
    "('ex_54dcdfd2-280d-4c2b-ae6b-c6089a985eee','Pec Fly','pec "
    "fly','reps','sets',0,'external','pec_fly','sp_pec_fly_v1','resolved','rear_delt_pec_fly'),"
    "('ex_58b8dfbc-92b2-4783-a449-9947a42480b8','Chin Assist','chin "
    "assist','reps','sets',0,'assistance','assisted','sp_assisted_chin_v1','resolved','assisted_"
    "dip_chin_machine'),"
    "('ex_44358a7b-09c8-4992-8f70-7eee4e99bbdf','Dip Assist','dip "
    "assist','reps','sets',0,'assistance','assisted','sp_assisted_dip_v1','resolved','assisted_dip_"
    "chin_machine');";

static const char *const MIGRATE_V12_TO_V13_SQL_C =
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT n.id,z.zone_id,z.role "
    "FROM exercises n JOIN exercises o ON o.exercise_id='ex_b432623f-bfe9-4daf-a653-60ec7fdffbde' "
    "JOIN exercise_body_zones z ON z.exercise_row_id=o.id WHERE "
    "n.exercise_id='ex_0e26c06f-a458-40a4-be20-4ed219ede30d';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT id,'chest','primary' "
    "FROM exercises WHERE exercise_id='ex_54dcdfd2-280d-4c2b-ae6b-c6089a985eee';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT "
    "id,'shoulders','secondary' FROM exercises WHERE "
    "exercise_id='ex_54dcdfd2-280d-4c2b-ae6b-c6089a985eee';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT id,'back','primary' FROM "
    "exercises WHERE exercise_id='ex_58b8dfbc-92b2-4783-a449-9947a42480b8';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT id,'arms','secondary' "
    "FROM exercises WHERE exercise_id='ex_58b8dfbc-92b2-4783-a449-9947a42480b8';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT id,'arms','primary' FROM "
    "exercises WHERE exercise_id='ex_44358a7b-09c8-4992-8f70-7eee4e99bbdf';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT id,'chest','secondary' "
    "FROM exercises WHERE exercise_id='ex_44358a7b-09c8-4992-8f70-7eee4e99bbdf';"
    "INSERT INTO exercise_body_zones(exercise_row_id,zone_id,role) SELECT "
    "id,'shoulders','secondary' FROM exercises WHERE "
    "exercise_id='ex_44358a7b-09c8-4992-8f70-7eee4e99bbdf';"
    "INSERT INTO exercise_body_zone_sync(exercise_row_id,synced_state) SELECT id,'thighs|glutes' "
    "FROM exercises WHERE exercise_id='ex_0e26c06f-a458-40a4-be20-4ed219ede30d';"
    "INSERT INTO exercise_body_zone_sync(exercise_row_id,synced_state) SELECT id,'chest|shoulders' "
    "FROM exercises WHERE exercise_id='ex_54dcdfd2-280d-4c2b-ae6b-c6089a985eee';"
    "INSERT INTO exercise_body_zone_sync(exercise_row_id,synced_state) SELECT id,'back|arms' FROM "
    "exercises WHERE exercise_id='ex_58b8dfbc-92b2-4783-a449-9947a42480b8';"
    "INSERT INTO exercise_body_zone_sync(exercise_row_id,synced_state) SELECT "
    "id,'arms|chest,shoulders' FROM exercises WHERE "
    "exercise_id='ex_44358a7b-09c8-4992-8f70-7eee4e99bbdf';"
    "UPDATE session_exercises SET exercise_row_id=(SELECT id FROM exercises WHERE "
    "exercise_id='ex_0e26c06f-a458-40a4-be20-4ed219ede30d') WHERE entry_id='sxe_draft_legacy_7' "
    "AND equipment_id='plate_loaded_leg_press' AND exercise_row_id=(SELECT id FROM exercises WHERE "
    "exercise_id='ex_b432623f-bfe9-4daf-a653-60ec7fdffbde');"
    "UPDATE session_exercises SET exercise_row_id=(SELECT id FROM exercises WHERE "
    "exercise_id='ex_f01d2a46-6984-4dec-8934-4d82fca6dfc2') WHERE entry_id IN "
    "('sxe_093c1331-beaa-4b69-91b3-240292709be6','sxe_f25142c8-455e-4346-9bfc-31d0989e275d','sxe_"
    "f2242691-ba6c-48a0-b938-a1f744375d75','sxe_9f8882f4-069e-49d5-8216-19d16b467e4a') AND "
    "equipment_id='treadmill' AND exercise_row_id=(SELECT id FROM exercises WHERE "
    "exercise_id='ex_b1e6ffc6-75b5-45ff-a3c0-e7433c58013d');"
    "UPDATE exercises SET "
    "load_semantics=NULL,machine_variant=NULL,machine_provenance=NULL,scientific_profile_id=NULL,"
    "science_state='unresolved',legacy_equipment_id=NULL WHERE "
    "exercise_id='ex_617007f9-7420-4408-91b9-8ffb77900f13';"
...
static const char *const MIGRATE_V13_TO_V14_SQL =
    ...
    "CREATE INDEX exercise_feedback_timeline ON "
    "exercise_feedback(session_exercise_row_id,observed_at,feedback_id);"
    "CREATE TABLE session_followups("
    "id INTEGER PRIMARY KEY,followup_id TEXT NOT NULL UNIQUE,"
    "session_row_id INTEGER NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,"
    "observed_at TEXT NOT NULL,raw_text TEXT NOT NULL CHECK(length(trim(raw_text))>0));"
    "CREATE INDEX session_followups_timeline ON "
    "session_followups(session_row_id,observed_at,followup_id);"
...
static const char *const MIGRATE_V14_TO_V15_SQL =
    ...
    "revision_id TEXT PRIMARY KEY,feedback_id TEXT NOT NULL REFERENCES "
    "exercise_feedback(feedback_id) ON DELETE CASCADE,"
    "created_at TEXT NOT NULL,raw_text TEXT NOT NULL CHECK(length(trim(raw_text))>0));"
    "CREATE INDEX exercise_feedback_revisions_current ON "
    "exercise_feedback_revisions(feedback_id,created_at,revision_id);"
    "CREATE TABLE session_followup_revisions("
    "revision_id TEXT PRIMARY KEY,followup_id TEXT NOT NULL REFERENCES "
    "session_followups(followup_id) ON DELETE CASCADE,"
    "created_at TEXT NOT NULL,raw_text TEXT NOT NULL CHECK(length(trim(raw_text))>0));"
    "CREATE INDEX session_followup_revisions_current ON "
    "session_followup_revisions(followup_id,created_at,revision_id);"
...
read_single_int_pragma(TrainlogDatabase *database, const char *sql, int *output);
...
static TrainlogStatus migrate_v15_to_v16(TrainlogDatabase *database) {
    ...
    int rc;
    if (read_single_int_pragma(database, "PRAGMA user_version;", &version) != TRAINLOG_STATUS_OK ||
        version != 15) {
    ...
    if (execute_sql(database, "PRAGMA foreign_keys=OFF;") != TRAINLOG_STATUS_OK ||
        ...
        execute_sql(
            ...
            "se.target_duration_seconds,se.target_weight_kg,se.equipment_id,se.notes,e.tracking_"
            "mode "
    ...
    rc = sqlite3_prepare_v2(database->connection,
                            ...
                            "se.tracking_mode<>e.tracking_mode;",
                            -1,
                            &statement,
                            NULL);
    ...
    if (read_single_int_pragma(database,
                               "SELECT COUNT(*) FROM session_exercises;",
                               &migrated_count) != TRAINLOG_STATUS_OK ||
        migrated_count != source_count || invalid != 0 ||
        read_single_int_pragma(database,
                               "SELECT COUNT(*) FROM pragma_foreign_key_check;",
                               &invalid) != TRAINLOG_STATUS_OK ||
        invalid != 0 ||
        execute_sql(database, "PRAGMA user_version=16;COMMIT;") != TRAINLOG_STATUS_OK) {
        goto rollback;
    }
...
static const char *const MIGRATE_V16_TO_V17_SQL =
    ...
    "INSERT INTO "
    "exercise_profile_state(exercise_row_id,revision_id,parent_revision_id,legacy_seed) "
    ...
    "tracking_mode TEXT NOT NULL,data_fields INTEGER NOT NULL,legacy_seed INTEGER NOT NULL "
    "CHECK(legacy_seed IN(0,1)),"
    "PRIMARY KEY(exercise_row_id,revision_id));"
    "INSERT INTO exercise_profile_revisions SELECT "
    "id,'pr_legacy_v1',NULL,recording_mode,tracking_mode,data_fields,1 FROM exercises;"
    "CREATE TRIGGER exercise_profile_state_insert AFTER INSERT ON exercises BEGIN "
    "INSERT INTO "
    "exercise_profile_state(exercise_row_id,revision_id,parent_revision_id,legacy_seed) "
    "VALUES(NEW.id,trainlog_profile_revision('pr_legacy_v1',NEW.recording_mode,NEW.tracking_mode,"
    "NEW.data_fields),'pr_legacy_v1',0);"
    "INSERT INTO exercise_profile_revisions "
    "VALUES(NEW.id,'pr_legacy_v1',NULL,NEW.recording_mode,NEW.tracking_mode,NEW.data_fields,1);"
    "INSERT INTO exercise_profile_revisions "
    "VALUES(NEW.id,trainlog_profile_revision('pr_legacy_v1',NEW.recording_mode,NEW.tracking_mode,"
    "NEW.data_fields),'pr_legacy_v1',NEW.recording_mode,NEW.tracking_mode,NEW.data_fields,0);END;"
    "CREATE TRIGGER exercise_profile_state_update AFTER UPDATE OF "
    "recording_mode,tracking_mode,data_fields ON exercises "
    "WHEN (OLD.recording_mode<>NEW.recording_mode OR OLD.tracking_mode<>NEW.tracking_mode OR "
    "OLD.data_fields<>NEW.data_fields) "
    "AND NOT EXISTS(SELECT 1 FROM exercise_profile_state s JOIN exercise_profile_revisions r ON "
    "r.exercise_row_id=s.exercise_row_id AND r.revision_id=s.revision_id WHERE "
    "s.exercise_row_id=NEW.id AND r.recording_mode=NEW.recording_mode AND "
    "r.tracking_mode=NEW.tracking_mode AND r.data_fields=NEW.data_fields) BEGIN "
    "SELECT CASE WHEN (SELECT COUNT(*) FROM exercise_profile_revisions WHERE "
    "exercise_row_id=NEW.id)>=32 THEN RAISE(ABORT,'profile revision history limit') END;"
    "INSERT OR IGNORE INTO exercise_profile_revisions SELECT "
    "NEW.id,s.revision_id,s.parent_revision_id,OLD.recording_mode,OLD.tracking_mode,OLD.data_"
    "fields,s.legacy_seed FROM exercise_profile_state s WHERE s.exercise_row_id=NEW.id;"
    "UPDATE exercise_profile_state SET "
    "parent_revision_id=revision_id,revision_id=trainlog_profile_revision(revision_id,NEW."
    "recording_mode,NEW.tracking_mode,NEW.data_fields),legacy_seed=0 WHERE exercise_row_id=NEW.id;"
    "INSERT INTO exercise_profile_revisions SELECT "
    "NEW.id,s.revision_id,s.parent_revision_id,NEW.recording_mode,NEW.tracking_mode,NEW.data_"
    "fields,0 FROM exercise_profile_state s WHERE s.exercise_row_id=NEW.id;END;"
...
static const char *const MIGRATE_V17_TO_V18_SQL =
    ...
    "id INTEGER PRIMARY KEY,draft_row_id INTEGER NOT NULL REFERENCES ai_session_drafts(id) ON "
    "DELETE CASCADE,"
    "entry_id TEXT NOT NULL UNIQUE,position INTEGER NOT NULL CHECK(position>=0),exercise_row_id "
    "INTEGER NOT NULL REFERENCES exercises(id) ON DELETE RESTRICT,"
    "source_exercise_id TEXT NOT NULL,recording_mode TEXT NOT NULL CHECK(recording_mode='sets'),"
    "tracking_mode TEXT NOT NULL CHECK(tracking_mode IN('reps','duration')),data_fields INTEGER "
    "NOT NULL,"
...
static const char *const MIGRATE_V18_TO_V19_SQL =
    ...
    "FOREIGN KEY(owner_kind,owner_id,revision_id) REFERENCES "
    "sync_note_revisions(owner_kind,owner_id,revision_id));"
    "CREATE TABLE execution_drafts("
    "session_id TEXT PRIMARY KEY,session_type TEXT NOT NULL CHECK(session_type "
    "IN('training','max_test')),"
    ...
    "session_id TEXT NOT NULL,revision_id TEXT NOT NULL,parent_revision_id TEXT,payload_json TEXT "
    "NOT NULL,"
...
static const char *const MIGRATE_V20_TO_V21_SQL =
    "BEGIN IMMEDIATE;"
    "CREATE TABLE sync_peer_identity(singleton INTEGER PRIMARY KEY CHECK(singleton=1),peer_id TEXT "
    "NOT NULL UNIQUE,kind TEXT NOT NULL CHECK(kind IN('desktop','android')));"
    "CREATE TABLE sync_generations(generation_id TEXT PRIMARY KEY,run_id TEXT NOT "
    "NULL,producer_peer_id TEXT NOT NULL,consumer_peer_id TEXT NOT NULL,producer_kind TEXT NOT "
    "NULL,generated_at TEXT NOT NULL,parent_generation_id TEXT,manifest_sha256 TEXT NOT "
    "NULL,status TEXT NOT NULL CHECK(status "
    "IN('captured','published','waiting_acknowledgement','acknowledged','rejected')),manifest_json "
    "TEXT NOT NULL,staging_path TEXT NOT NULL,acknowledged_at TEXT);"
    "CREATE TABLE sync_generation_artifacts(generation_id TEXT NOT NULL REFERENCES "
    "sync_generations(generation_id) ON DELETE CASCADE,logical_name TEXT NOT NULL,format TEXT NOT "
    "NULL,version INTEGER NOT NULL,filename TEXT NOT NULL,size_bytes INTEGER NOT NULL,sha256 TEXT "
    "NOT NULL,required INTEGER NOT NULL CHECK(required IN(0,1)),PRIMARY "
    "KEY(generation_id,logical_name),UNIQUE(generation_id,filename));"
    "CREATE TABLE sync_consumed_generations(generation_id TEXT PRIMARY KEY,run_id TEXT NOT "
    "NULL,producer_peer_id TEXT NOT NULL,consumer_peer_id TEXT NOT NULL,parent_generation_id "
    "TEXT,manifest_sha256 TEXT NOT NULL,consumed_at TEXT NOT NULL,result TEXT NOT NULL "
    "CHECK(result IN('consumed','rejected')),durability TEXT NOT NULL,diagnostic TEXT NOT "
    "NULL,ack_json TEXT NOT NULL,UNIQUE(producer_peer_id,generation_id));"
    "CREATE TABLE sync_acknowledgements(ack_id TEXT PRIMARY KEY,generation_id TEXT NOT NULL,run_id "
    "TEXT NOT NULL,producer_peer_id TEXT NOT NULL,consumer_peer_id TEXT NOT NULL,manifest_sha256 "
    "TEXT NOT NULL,result TEXT NOT NULL CHECK(result IN('consumed','rejected')),durability TEXT "
    "NOT NULL,created_at TEXT NOT NULL,diagnostic TEXT NOT NULL,payload_sha256 TEXT NOT NULL);"
    "CREATE TABLE sync_causal_publications(operation_id TEXT NOT NULL REFERENCES "
    "sync_causal_operations(operation_id),generation_id TEXT NOT NULL,first_emission INTEGER NOT "
    "NULL CHECK(first_emission IN(0,1)),PRIMARY KEY(operation_id,generation_id));"
...
static const char *const MIGRATE_V22_TO_V23_SQL =
    ...
    "delivery_state TEXT NOT NULL DEFAULT 'local' CHECK(delivery_state "
    "IN('local','pending','acknowledged','remote_unknown','cancelled')),"
    "source_proposal_id TEXT,source_payload_sha256 TEXT);"
    "CREATE TABLE session_preparation_revisions("
    "revision_id TEXT PRIMARY KEY,preparation_id TEXT NOT NULL REFERENCES "
    "session_preparations(preparation_id) ON DELETE RESTRICT,"
    "parent_revision_id TEXT,title TEXT NOT NULL,session_type TEXT NOT NULL CHECK(session_type "
    "IN('training','max_test')),"
    "planned_for TEXT,notes TEXT,created_at TEXT NOT NULL,"
    "CHECK(length(CAST(title AS BLOB))<=200),CHECK(notes IS NULL OR length(CAST(notes AS "
    "BLOB))<=4000),"
    "UNIQUE(preparation_id,revision_id));"
    "CREATE TABLE session_preparation_entries("
    "revision_id TEXT NOT NULL REFERENCES session_preparation_revisions(revision_id) ON DELETE "
    "RESTRICT,"
    ...
    "tracking_mode TEXT NOT NULL CHECK(tracking_mode IN('reps','duration')),data_fields INTEGER "
    "NOT NULL,"
    "load_mode TEXT NOT NULL CHECK(load_mode IN('none','external','assistance')),rest_seconds "
    "INTEGER NOT NULL CHECK(rest_seconds BETWEEN 0 AND 86400),"
    "target_sets INTEGER,target_reps INTEGER,target_duration_seconds INTEGER,target_weight_kg "
    "REAL,notes TEXT,"
    ...
    "request_id TEXT PRIMARY KEY,command TEXT NOT NULL,preparation_id TEXT NOT NULL,revision_id "
    "TEXT NOT NULL,"
    "response_json TEXT NOT NULL,created_at TEXT NOT NULL);"
    "CREATE TABLE session_preparation_deliveries("
    "delivery_id TEXT PRIMARY KEY,preparation_id TEXT NOT NULL REFERENCES "
    "session_preparations(preparation_id),"
    "revision_id TEXT NOT NULL REFERENCES "
    "session_preparation_revisions(revision_id),execution_session_id TEXT NOT NULL UNIQUE,"
...
/* WHY: reusable programs are planning definitions, not performed sessions or
 * executable drafts. CONTRACT: v25 stores imported Program V1 documents and
 * their ordered definitions separately, and records optional provenance on a
 * preparation created from one definition. INVARIANT: migration invents no
 * program, preparation, delivery, draft, or performed workout. */
static const char *const MIGRATE_V24_TO_V25_SQL =
    "BEGIN IMMEDIATE;"
    "CREATE TABLE IF NOT EXISTS programs("
    "program_id TEXT PRIMARY KEY,title TEXT NOT NULL,note TEXT,"
    "state TEXT NOT NULL CHECK(state IN('active','archived')),"
    "start_date TEXT,end_date TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,"
    "revision_id TEXT NOT NULL,source_format TEXT NOT NULL,source_version INTEGER NOT NULL,"
    "source_payload_sha256 TEXT NOT NULL,"
    "CHECK(length(CAST(title AS BLOB)) BETWEEN 1 AND 200),"
    "CHECK(note IS NULL OR length(CAST(note AS BLOB))<=4000),"
    "CHECK(length(source_payload_sha256)=64),UNIQUE(program_id,source_payload_sha256));"
    ...
    "planned_for TEXT,note TEXT,"
    "CHECK(length(CAST(title AS BLOB)) BETWEEN 1 AND 200),"
    "CHECK(note IS NULL OR length(CAST(note AS BLOB))<=4000),"
    "UNIQUE(program_id,position));"
    "CREATE TABLE IF NOT EXISTS program_session_entries("
    "program_session_id TEXT NOT NULL REFERENCES program_sessions(program_session_id) "
    "ON DELETE RESTRICT,"
    "entry_id TEXT NOT NULL,position INTEGER NOT NULL CHECK(position>=0),"
    "exercise_id TEXT NOT NULL,equipment_id TEXT,"
    "recording_mode TEXT NOT NULL CHECK(recording_mode IN('sets','continuous')),"
    "tracking_mode TEXT NOT NULL CHECK(tracking_mode IN('reps','duration')),"
    "data_fields INTEGER NOT NULL,"
    "load_mode TEXT NOT NULL CHECK(load_mode IN('none','external','assistance')),"
    "rest_seconds INTEGER NOT NULL CHECK(rest_seconds BETWEEN 0 AND 86400),"
    "target_sets INTEGER,target_reps INTEGER,target_duration_seconds INTEGER,"
    "target_weight_kg REAL,notes TEXT,"
    "CHECK(notes IS NULL OR length(CAST(notes AS BLOB))<=4000),"
    "PRIMARY KEY(program_session_id,entry_id),UNIQUE(program_session_id,position));"
    "CREATE INDEX IF NOT EXISTS program_session_entries_exercise "
    "ON program_session_entries(exercise_id);"
...
static const char *const MIGRATE_V29_TO_V30_SQL =
    ...
    "capture_id TEXT PRIMARY KEY,context_kind TEXT NOT NULL CHECK(context_kind "
    "IN('session','cardio','sleep')),"
 ...
 * widened to cardio. */
static const char *const MIGRATE_V31_TO_V32_SQL = "BEGIN IMMEDIATE;"
...
static const char *const MIGRATE_V33_TO_V34_SQL =
    ...
    "run_id TEXT PRIMARY KEY,session_id TEXT NOT NULL UNIQUE REFERENCES sessions(session_id) ON "
    "DELETE RESTRICT,"
    "started_at TEXT NOT NULL,ended_at TEXT NOT NULL,imported_at TEXT NOT NULL,"
    ...
    "phase_id TEXT NOT NULL UNIQUE,entry_id TEXT NOT NULL,position INTEGER NOT NULL "
    "CHECK(position>=0),"
    ...
    "exit_kind TEXT NOT NULL CHECK(exit_kind "
    "IN('fixed_duration','enter_target','recover_below','duration_or_recover')),"
    "exit_seconds INTEGER,exit_bpm INTEGER,started_at TEXT NOT NULL,ended_at TEXT NOT NULL,"
    "final_instruction TEXT NOT NULL CHECK(final_instruction "
    "IN('accelerate','maintain','slow_down','suspended')),"
    ...
    "(exit_kind='enter_target' AND exit_seconds IS NULL AND exit_bpm IS NULL AND target_min_bpm IS "
    "NOT NULL) OR "
    ...
    "FOREIGN KEY(run_id,phase_id) REFERENCES cardio_guidance_phases(run_id,phase_id) ON DELETE "
    "CASCADE,"
    "CHECK((target_min_bpm IS NULL AND target_max_bpm IS NULL) OR "
    "(target_min_bpm BETWEEN 1 AND 65535 AND target_max_bpm BETWEEN target_min_bpm AND 65535)));"
...
static TrainlogStatus ensure_v35_medication_quantity(TrainlogDatabase *database) {
    sqlite3_stmt *statement = NULL;
    bool found = false;
    int result = sqlite3_prepare_v2(
        database->connection, "PRAGMA table_info(sleep_medication_intakes);", -1, &statement, NULL);
    if (result != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    while ((result = sqlite3_step(statement)) == SQLITE_ROW) {
        const char *name = (const char *)sqlite3_column_text(statement, 1);
        if (name != NULL && strcmp(name, "quantity") == 0) {
            found = true;
        }
    }
    if (result != SQLITE_DONE || sqlite3_finalize(statement) != SQLITE_OK) {
    ...
    if (!found &&
        execute_sql(database,
                    "ALTER TABLE sleep_medication_intakes ADD COLUMN quantity INTEGER NOT NULL "
                    "DEFAULT 1 CHECK(quantity BETWEEN 1 AND 99);") != TRAINLOG_STATUS_OK) {
        (void)execute_sql(database, "ROLLBACK;");
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    if (execute_sql(database, "PRAGMA user_version=35;") != TRAINLOG_STATUS_OK ||
        execute_sql(database, "COMMIT;") != TRAINLOG_STATUS_OK) {
        (void)execute_sql(database, "ROLLBACK;");
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    return TRAINLOG_STATUS_OK;
...
static TrainlogStatus ensure_v25_preparation_provenance(TrainlogDatabase *database) {
    sqlite3_stmt *statement = NULL;
    bool has_program_id = false;
    bool has_program_session_id = false;
    int result;

    result = sqlite3_prepare_v2(
        database->connection, "PRAGMA table_info(session_preparations);", -1, &statement, NULL);
    if (result != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    while ((result = sqlite3_step(statement)) == SQLITE_ROW) {
        const char *name = (const char *)sqlite3_column_text(statement, 1);
        if (name != NULL && strcmp(name, "source_program_id") == 0) {
            has_program_id = true;
        }
        if (name != NULL && strcmp(name, "source_program_session_id") == 0) {
            has_program_session_id = true;
        }
    }
    if (result != SQLITE_DONE || sqlite3_finalize(statement) != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    ...
    if (!has_program_id &&
        execute_sql(database,
                    "ALTER TABLE session_preparations ADD COLUMN source_program_id TEXT "
                    "REFERENCES programs(program_id);") != TRAINLOG_STATUS_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    if (!has_program_session_id &&
        execute_sql(database,
                    "ALTER TABLE session_preparations ADD COLUMN source_program_session_id TEXT "
                    "REFERENCES program_sessions(program_session_id);") != TRAINLOG_STATUS_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    return TRAINLOG_STATUS_OK;
}
...
static TrainlogStatus ensure_v18_ai_draft_publication_state(TrainlogDatabase *database) {
    sqlite3_stmt *statement = NULL;
    int rc;
    bool found = false;
    ...
    if (!found &&
        execute_sql(database, "ALTER TABLE ai_session_drafts ADD COLUMN published_at TEXT;") !=
            TRAINLOG_STATUS_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
...
static const char *const ENSURE_V17_PROFILE_REVISIONS_SQL =
    ...
    "SELECT "
    "e.id,s.revision_id,s.parent_revision_id,e.recording_mode,e.tracking_mode,e.data_fields,s."
    "legacy_seed "
    "FROM exercises e JOIN exercise_profile_state s ON s.exercise_row_id=e.id;"
    "CREATE TRIGGER exercise_profile_state_insert AFTER INSERT ON exercises BEGIN "
    "INSERT INTO exercise_profile_state "
    "VALUES(NEW.id,trainlog_profile_revision('pr_legacy_v1',NEW.recording_mode,NEW.tracking_mode,"
    "NEW.data_fields),'pr_legacy_v1',0);"
    "INSERT INTO exercise_profile_revisions "
    "VALUES(NEW.id,'pr_legacy_v1',NULL,NEW.recording_mode,NEW.tracking_mode,NEW.data_fields,1);"
...
static TrainlogStatus normalize_v17_prototype_profiles(TrainlogDatabase *database) {
    ...
    static const char *const VALIDATION_SQL =
        ...
        " WHERE s.revision_id GLOB 'pr1*' AND (s.parent_revision_id IS NOT NULL OR "
        "s.legacy_seed<>1 OR "
        ...
        " WHERE r.revision_id GLOB 'pr1*' AND (s.exercise_row_id IS NULL OR r.parent_revision_id "
        "IS NOT NULL OR "
        " r.legacy_seed<>1 OR r.recording_mode<>(SELECT recording_mode FROM exercises WHERE "
        "id=r.exercise_row_id) OR "
        ...
        " (SELECT COUNT(*) FROM exercise_profile_revisions r WHERE "
        "r.exercise_row_id=s.exercise_row_id)<>1)"
        ");";
    static const char *const NORMALIZE_SQL =
        ...
        "INSERT INTO exercise_profile_revisions "
        "SELECT "
        "e.id,trainlog_profile_revision('pr_legacy_v1',e.recording_mode,e.tracking_mode,e.data_"
        "fields),"
...
static bool bind_initial_body_zone(sqlite3_stmt *statement,
                                   const char *exercise_id,
                                   const char *zone_id,
                                   const char *role) {
    int rc;
...
static TrainlogStatus migrate_v10_to_v11(TrainlogDatabase *database) {
    ...
    for (index = 0U; index < trainlog_body_zone_initial_mapping_count(); ++index) {
        ...
        const char *cursor;
        if (mapping == NULL ||
            !bind_initial_body_zone(
                statement, mapping->exercise_id, mapping->primary_zone_id, "primary")) {
            goto rollback;
        }
        ...
        while (cursor != NULL && cursor[0] != '\0') {
            ...
            if (length == 0U || length > TRAINLOG_ZONE_ID_MAX) {
                goto rollback;
            }
            (void)memcpy(zone_id, cursor, length);
            zone_id[length] = '\0';
            if (!bind_initial_body_zone(statement, mapping->exercise_id, zone_id, "secondary")) {
                goto rollback;
            }
...
read_single_int_pragma(TrainlogDatabase *database, const char *sql, int *output) {
    ...
    if (database == NULL || database->connection == NULL || sql == NULL || output == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
...
static TrainlogStatus list_feedback_query(TrainlogDatabase *database,
                                          const char *sql,
                                          const char *session_id,
                                          TrainlogFeedbackView *output,
                                          size_t capacity,
                                          size_t *output_count,
                                          bool has_entry) {
    sqlite3_stmt *statement = NULL;
    ...
    while (count < capacity && (rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        (void)snprintf(output[count].entry_id,
                       sizeof(output[count].entry_id),
                       "%s",
...
TrainlogStatus trainlog_database_list_training_feedback(TrainlogDatabase *database,
                                                        const char *session_id,
                                                        TrainlogFeedbackView *exercise_feedback,
                                                        size_t exercise_capacity,
                                                        size_t *exercise_count,
                                                        TrainlogFeedbackView *followups,
                                                        size_t followup_capacity,
                                                        size_t *followup_count) {
    TrainlogStatus status = list_feedback_query(
        database,
        "SELECT f.feedback_id,se.entry_id,f.observed_at,f.raw_text FROM exercise_feedback f "
        "JOIN session_exercises se ON se.id=f.session_exercise_row_id JOIN sessions s ON "
        "s.id=se.session_row_id "
...
static TrainlogStatus initialize_or_validate_schema(TrainlogDatabase *database,
    ...
    if (version > TRAINLOG_DATABASE_SCHEMA_VERSION) {
        ...
        set_application_diagnostic(output_diagnostic, output_diagnostic_capacity, message);
        return TRAINLOG_STATUS_SCHEMA_UNSUPPORTED;
    }

    if (version == 0) {
        status = execute_sql(database, SCHEMA_V7_SQL_A);

        if (status == TRAINLOG_STATUS_OK) {
            status = execute_sql(database, SCHEMA_V7_SQL_B);
        }
        if (status == TRAINLOG_STATUS_OK) {
        ...
        if (version == 1) {
            status = execute_sql(database, MIGRATE_V1_TO_V3_SQL);

            if (status == TRAINLOG_STATUS_OK) {
                status = execute_sql(database, MIGRATE_V3_TO_V4_SQL_A);
            }

            if (status == TRAINLOG_STATUS_OK) {
                status = execute_sql(database, MIGRATE_V3_TO_V4_SQL_B);
            }
        } else if (version == 2) {
            status = execute_sql(database, MIGRATE_V2_TO_V3_SQL);
        ...
        if (status == TRAINLOG_STATUS_OK) {
            status = execute_sql(database, MIGRATE_V4_TO_V5_SQL_A);
        }

        if (status == TRAINLOG_STATUS_OK) {
            status = execute_sql(database, MIGRATE_V4_TO_V5_SQL_B);
        }
    ...
    if (status == TRAINLOG_STATUS_OK && version < 25) {
        status = execute_sql(database, MIGRATE_V24_TO_V25_SQL);
    }
    if (status == TRAINLOG_STATUS_OK) {
    ...
    if (status == TRAINLOG_STATUS_OK) {
        status = execute_sql(database, "BEGIN IMMEDIATE;");
    }
    if (status == TRAINLOG_STATUS_OK) {
        status = execute_sql(database, ENSURE_V17_PROFILE_REVISIONS_SQL);
    }
    if (status == TRAINLOG_STATUS_OK) {
        status = normalize_v17_prototype_profiles(database);
    }
    if (status == TRAINLOG_STATUS_OK) {
        status = execute_sql(database, "COMMIT;");
    }
...
TrainlogStatus trainlog_database_open(const char *path, TrainlogDatabase **output_database) {
    return trainlog_database_open_with_diagnostic(path, output_database, NULL, 0U);
}

TrainlogStatus trainlog_database_open_with_diagnostic(const char *path,
    ...
    *output_database = NULL;
    ...
    if (sqlite3_create_function(database->connection,
                                "trainlog_profile_revision",
                                4,
                                SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                                NULL,
                                sqlite_profile_revision,
                                NULL,
                                NULL) != SQLITE_OK) {
        set_open_diagnostic(output_diagnostic,
                            output_diagnostic_capacity,
                            "register profile revision function",
                            database->connection,
                            SQLITE_ERROR);
        trainlog_database_close(database);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    ...
    if (sqlite3_create_function(database->connection,
                                ...
                                SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                                NULL,
                                sqlite_prototype_profile_valid,
                                NULL,
                                NULL) != SQLITE_OK) {
        set_open_diagnostic(output_diagnostic,
                            output_diagnostic_capacity,
                            "register prototype profile validator",
                            database->connection,
                            SQLITE_ERROR);
        trainlog_database_close(database);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    ...
    status = initialize_or_validate_schema(database, output_diagnostic, output_diagnostic_capacity);
    if (status != TRAINLOG_STATUS_OK) {
        trainlog_database_close(database);
        return status;
    }
...
TrainlogStatus trainlog_database_schema_version(TrainlogDatabase *database, int *output_version) {
    return read_single_int_pragma(database, "PRAGMA user_version;", output_version);
}

TrainlogStatus trainlog_database_foreign_keys_enabled(TrainlogDatabase *database,
                                                      int *output_enabled) {
    return read_single_int_pragma(database, "PRAGMA foreign_keys;", output_enabled);
}

TrainlogStatus trainlog_database_create_custom_equipment(TrainlogDatabase *database,
    ...
    rc = sqlite3_prepare_v2(database->connection,
                            "INSERT INTO "
                            "custom_equipment(equipment_id,display_name,label_name,equipment_type,"
                            "load_semantics) VALUES(?1,?2,?3,?4,?5);",
                            -1,
                            &statement,
                            NULL);
    if (rc != SQLITE_OK) {
...
TrainlogStatus trainlog_database_list_custom_equipment(TrainlogDatabase *database,
    ...
    *output_count = 0U;
    rc = sqlite3_prepare_v2(
        database->connection,
        "SELECT equipment_id,display_name,label_name,equipment_type,load_semantics FROM "
        "custom_equipment ce "
        "WHERE NOT EXISTS(SELECT 1 FROM sync_causal_state s WHERE s.target_kind='custom_equipment' "
        "AND s.target_id=ce.equipment_id AND s.deleted=1) "
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (count < capacity) {
            ...
            (void)snprintf(item->equipment_id,
                           sizeof(item->equipment_id),
                           "%s",
                           sqlite3_column_text(statement, 0));
            (void)snprintf(item->display_name,
                           sizeof(item->display_name),
                           "%s",
                           sqlite3_column_text(statement, 1));
            (void)snprintf(item->label_name,
                           sizeof(item->label_name),
                           "%s",
                           sqlite3_column_text(statement, 2));
            (void)snprintf(item->equipment_type,
                           sizeof(item->equipment_type),
                           "%s",
                           sqlite3_column_text(statement, 3));
            (void)snprintf(item->load_semantics,
...
static bool copy_custom_equipment_page_column(sqlite3_stmt *statement,
                                              int column,
                                              char *output,
                                              size_t capacity) {
...
TrainlogStatus trainlog_database_list_custom_equipment_page(TrainlogDatabase *database,
                                                            size_t offset,
                                                            TrainlogCustomEquipment *output,
                                                            size_t capacity,
                                                            size_t *output_count,
    ...
    static const char *const SQL =
        ...
        "FROM custom_equipment ce WHERE NOT EXISTS(SELECT 1 FROM sync_causal_state s WHERE "
        "s.target_kind='custom_equipment' AND s.target_id=ce.equipment_id AND s.deleted=1) ORDER "
        "BY display_name COLLATE NOCASE,equipment_id "
        "LIMIT ?1 OFFSET ?2;";
    sqlite3_stmt *statement = NULL;
    size_t count = 0U;
    int rc;

    if (output != NULL && output_count != NULL && output_more != NULL && capacity >= 1U &&
        capacity <= TRAINLOG_CUSTOM_EQUIPMENT_PAGE_MAX) {
        *output_count = 0U;
        *output_more = false;
    }
    if (database == NULL || output == NULL || output_count == NULL || output_more == NULL ||
        capacity == 0U || capacity > TRAINLOG_CUSTOM_EQUIPMENT_PAGE_MAX ||
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        (void)memset(&item, 0, sizeof(item));
        if (!copy_custom_equipment_page_column(
                statement, 0, item.equipment_id, sizeof(item.equipment_id)) ||
            !copy_custom_equipment_page_column(
                statement, 1, item.display_name, sizeof(item.display_name)) ||
            !copy_custom_equipment_page_column(
                statement, 2, item.label_name, sizeof(item.label_name)) ||
            !copy_custom_equipment_page_column(
                statement, 3, item.equipment_type, sizeof(item.equipment_type)) ||
            !copy_custom_equipment_page_column(
                statement, 4, item.load_semantics, sizeof(item.load_semantics))) {
            (void)sqlite3_finalize(statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
        if (count == capacity) {
...
TrainlogStatus trainlog_database_resolve_equipment(TrainlogDatabase *database,
                                                   const char *equipment_id,
                                                   TrainlogResolvedEquipment *output) {
    const TrainlogEquipment *supplied;
    sqlite3_stmt *statement = NULL;
    int rc;
    if (database == NULL || equipment_id == NULL || equipment_id[0] == '\0' || output == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (supplied != NULL) {
        (void)snprintf(
            output->display_name, sizeof(output->display_name), "%s", supplied->display_name);
        (void)snprintf(output->label_name, sizeof(output->label_name), "%s", supplied->label_name);
        (void)snprintf(
            output->equipment_type, sizeof(output->equipment_type), "%s", supplied->equipment_type);
        (void)snprintf(
            output->load_semantics, sizeof(output->load_semantics), "%s", supplied->load_semantics);
        output->origin = TRAINLOG_EQUIPMENT_SUPPLIED;
        return TRAINLOG_STATUS_OK;
    }
    rc = sqlite3_prepare_v2(database->connection,
                            "SELECT display_name,label_name,equipment_type,load_semantics FROM "
                            "custom_equipment WHERE equipment_id=?1;",
                            -1,
                            &statement,
                            NULL);
    if (rc != SQLITE_OK) {
    ...
    rc = sqlite3_step(statement);
    if (rc == SQLITE_ROW) {
        (void)snprintf(output->display_name,
                       sizeof(output->display_name),
                       "%s",
                       sqlite3_column_text(statement, 0));
        (void)snprintf(output->label_name,
                       sizeof(output->label_name),
                       "%s",
                       sqlite3_column_text(statement, 1));
        (void)snprintf(output->equipment_type,
                       sizeof(output->equipment_type),
                       "%s",
                       sqlite3_column_text(statement, 2));
        (void)snprintf(output->load_semantics,
                       sizeof(output->load_semantics),
                       "%s",
                       sqlite3_column_text(statement, 3));
        ...
        (void)snprintf(
            output->display_name, sizeof(output->display_name), "Inconnu (%s)", equipment_id);
...
TrainlogStatus trainlog_database_list_exercise_equipment(TrainlogDatabase *database,
                                                         const char *exercise_id,
                                                         TrainlogResolvedEquipment *output,
                                                         size_t capacity,
                                                         size_t *output_count) {
    ...
    *output_count = 0U;
    rc = sqlite3_prepare_v2(
        database->connection,
        "SELECT DISTINCT se.equipment_id FROM session_exercises se JOIN exercises e ON "
        "e.id=se.exercise_row_id WHERE e.exercise_id=?1 AND se.equipment_id IS NOT NULL AND "
        "se.equipment_id<>'' ORDER BY se.equipment_id;",
        -1,
        &statement,
        NULL);
    if (rc != SQLITE_OK) {
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        if (count < capacity && trainlog_database_resolve_equipment(database, id, &output[count]) !=
                                    TRAINLOG_STATUS_OK) {
            (void)sqlite3_finalize(statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
...
static const char *recording_mode_to_sql(TrainlogRecordingMode mode) {
    switch (mode) {
...
static bool exercise_profile_valid(TrainlogTrackingMode tracking_mode,
                                   TrainlogRecordingMode recording_mode,
                                   TrainlogExerciseDataFields data_fields) {
    if ((data_fields & ~TRAINLOG_EXERCISE_DATA_KNOWN_MASK) != 0U) {
        return false;
    }
    ...
    return recording_mode == TRAINLOG_RECORDING_SETS &&
           (tracking_mode == TRAINLOG_TRACKING_REPS || tracking_mode == TRAINLOG_TRACKING_DURATION);
}
...
static const char *session_type_to_sql(TrainlogSessionType type) {
    switch (type) {
...
TrainlogStatus trainlog_database_insert_exercise_profiled(TrainlogDatabase *database,
                                                          const char *exercise_id,
                                                          const char *name,
                                                          ...
                                                          TrainlogRecordingMode recording_mode,
                                                          TrainlogExerciseDataFields data_fields) {
    static const char *const SQL = "INSERT INTO exercises("
    ...
    if (database == NULL || database->connection == NULL || exercise_id == NULL ||
        exercise_id[0] == '\0' || name == NULL || name[0] == '\0' || normalized_name == NULL ||
        normalized_name[0] == '\0' ||
...
TrainlogStatus trainlog_database_insert_exercise(TrainlogDatabase *database,
                                                 const char *exercise_id,
                                                 const char *name,
                                                 const char *normalized_name,
                                                 TrainlogTrackingMode tracking_mode) {
    return trainlog_database_insert_exercise_profiled(
        database, exercise_id, name, normalized_name, tracking_mode, TRAINLOG_RECORDING_SETS, 0U);
}
...
TrainlogStatus trainlog_database_update_exercise_profiled(TrainlogDatabase *database,
    ...
    if (database == NULL || exercise_id == NULL || name == NULL || normalized_name == NULL ||
        name[0] == '\0' || normalized_name[0] == '\0' || strlen(name) > TRAINLOG_NAME_MAX ||
        !exercise_profile_valid(tracking_mode, recording_mode, data_fields)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    tracking = tracking_mode_to_sql(tracking_mode);
    ...
    rc = sqlite3_prepare_v2(database->connection,
                            ...
                            "recording_mode=?4,data_fields=?5 WHERE id=?6;",
                            -1,
                            &statement,
                            NULL);
    if (rc != SQLITE_OK ||
    ...
    if (rc != SQLITE_DONE) {
        status =
            rc == SQLITE_CONSTRAINT ? TRAINLOG_STATUS_CONFLICT : TRAINLOG_STATUS_DATABASE_ERROR;
        goto rollback;
    }
    if (sqlite3_finalize(statement) != SQLITE_OK) {
        statement = NULL;
    ...
    statement = NULL;
    status = trainlog_database_replace_exercise_body_zones(
        database, exercise_id, primary_zone_id, secondary_zone_ids, secondary_count);
    if (status != TRAINLOG_STATUS_OK) {
        goto rollback;
    }
    if (!owns_transaction) {
...
static TrainlogStatus
count_query(TrainlogDatabase *database, const char *sql, size_t *output_count) {
    sqlite3_stmt *statement = NULL;
    sqlite3_int64 count;
    int rc;

    if (database == NULL || database->connection == NULL || sql == NULL || output_count == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
...
TrainlogStatus trainlog_database_exercise_count(TrainlogDatabase *database, size_t *output_count) {
    return count_query(database, "SELECT COUNT(*) FROM exercises;", output_count);
}

TrainlogStatus trainlog_database_session_count(TrainlogDatabase *database, size_t *output_count) {
    return count_query(database, "SELECT COUNT(*) FROM sessions;", output_count);
}

static TrainlogTrackingMode tracking_mode_from_sql(const char *text) {
    return text != NULL && strcmp(text, "duration") == 0 ? TRAINLOG_TRACKING_DURATION
                                                         : TRAINLOG_TRACKING_REPS;
}

static TrainlogRecordingMode recording_mode_from_sql(const char *text) {
    return text != NULL && strcmp(text, "continuous") == 0 ? TRAINLOG_RECORDING_CONTINUOUS
                                                           : TRAINLOG_RECORDING_SETS;
}

static bool session_type_from_sql(const char *text, TrainlogSessionType *output) {
...
TrainlogStatus trainlog_database_list_exercises(TrainlogDatabase *database,
                                                TrainlogExercise *output,
                                                size_t capacity,
                                                size_t *output_count) {
    static const char *const SQL = "SELECT exercise_id, name, tracking_mode, "
                                   "recording_mode, data_fields "
                                   "FROM exercises "
    ...
    *output_count = 0U;
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (output != NULL && count < capacity) {
            const unsigned char *exercise_id = sqlite3_column_text(statement, 0);
            const unsigned char *name = sqlite3_column_text(statement, 1);
            const unsigned char *tracking = sqlite3_column_text(statement, 2);
            const unsigned char *recording = sqlite3_column_text(statement, 3);
            sqlite3_int64 data_fields = sqlite3_column_int64(statement, 4);

            if (exercise_id == NULL || name == NULL || tracking == NULL || recording == NULL ||
                data_fields < 0 || (uint64_t)data_fields > (uint64_t)UINT32_MAX ||
                (((TrainlogExerciseDataFields)data_fields) & ~TRAINLOG_EXERCISE_DATA_KNOWN_MASK) !=
                    0U) {
                (void)sqlite3_finalize(statement);
                return TRAINLOG_STATUS_DATABASE_ERROR;
            }
            ...
            (void)snprintf(output[count].exercise_id,
                           sizeof(output[count].exercise_id),
                           "%s",
                           (const char *)exercise_id);

            (void)snprintf(
                output[count].name, sizeof(output[count].name), "%s", (const char *)name);

            output[count].tracking_mode = tracking_mode_from_sql((const char *)tracking);

            output[count].recording_mode = recording_mode_from_sql((const char *)recording);

            output[count].data_fields = (TrainlogExerciseDataFields)data_fields;
        }
...
TrainlogStatus trainlog_database_resolve_exercise_id(TrainlogDatabase *database,
                                                     const char *exercise_id,
                                                     char *output_canonical_id,
                                                     size_t output_capacity) {
    static const char *const SQL = "SELECT exercise_id FROM exercises WHERE exercise_id=?1 "
...
static TrainlogStatus merge_bind_ids(TrainlogDatabase *database,
                                     const char *sql,
    ...
    if (rc != SQLITE_DONE) {
        (void)sqlite3_finalize(statement);
        return rc == SQLITE_CONSTRAINT ? TRAINLOG_STATUS_CONFLICT : TRAINLOG_STATUS_DATABASE_ERROR;
    }
    return sqlite3_finalize(statement) == SQLITE_OK ? TRAINLOG_STATUS_OK
                                                    : TRAINLOG_STATUS_DATABASE_ERROR;
}

TrainlogStatus trainlog_database_merge_exercises(TrainlogDatabase *database,
    ...
    TrainlogStatus status = TRAINLOG_STATUS_DATABASE_ERROR;
    int rc;
    if (database == NULL || database->connection == NULL || source_exercise_id == NULL ||
        canonical_exercise_id == NULL || source_exercise_id[0] == '\0' ||
        canonical_exercise_id[0] == '\0' ||
    ...
    status = merge_bind_ids(
        database, "DELETE FROM exercises WHERE id=?1;", source_row_id, canonical_row_id);
    if (status != TRAINLOG_STATUS_OK) {
        goto rollback;
    }
    if (execute_sql(database, "COMMIT;") != TRAINLOG_STATUS_OK) {
...
TrainlogStatus trainlog_database_preview_exercise_merge(TrainlogDatabase *database,
                                                        const char *source_exercise_id,
    ...
    int rc;
    if (database == NULL || database->connection == NULL || source_exercise_id == NULL ||
        source_exercise_id[0] == '\0' || output == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    rc = sqlite3_prepare_v2(database->connection, SQL, -1, &statement, NULL);
    if (rc != SQLITE_OK ||
...
static bool assignable_body_zone(const char *zone_id) {
    ...
    if (zone_id == NULL || zone_id[0] == '\0' || strlen(zone_id) > TRAINLOG_ZONE_ID_MAX) {
        return false;
    }
...
static TrainlogStatus insert_body_zone_relation(TrainlogDatabase *database,
...
TrainlogStatus trainlog_database_replace_exercise_body_zones(TrainlogDatabase *database,
                                                             const char *exercise_id,
                                                             const char *primary_zone_id,
                                                             const char *const *secondary_zone_ids,
                                                             size_t secondary_count) {
    ...
    if (database == NULL || database->connection == NULL || exercise_id == NULL ||
        ...
        (secondary_count > 0U && (primary_zone_id == NULL || primary_zone_id[0] == '\0')) ||
    ...
    for (index = 0U; index < secondary_count; ++index) {
        status =
            insert_body_zone_relation(database, row_id, secondary_zone_ids[index], "secondary");
        if (status != TRAINLOG_STATUS_OK) {
            goto finish;
        }
    }
...
TrainlogStatus trainlog_database_list_exercise_body_zones(TrainlogDatabase *database,
                                                          const char *exercise_id,
    ...
    *output_count = 0U;
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        if (zone_id == NULL || role == NULL || zone == NULL || zone->is_group ||
        ...
        if (count < capacity) {
            (void)memset(&output[count], 0, sizeof(output[count]));
            (void)snprintf(output[count].zone_id, sizeof(output[count].zone_id), "%s", zone_id);
            output[count].role = strcmp(role, "primary") == 0 ? TRAINLOG_BODY_ZONE_PRIMARY
...
static bool exercise_row_matches_body_zone(TrainlogDatabase *database,
                                           sqlite3_int64 row_id,
                                           const char *zone_id,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        if (candidate == NULL || role == NULL || zone == NULL || zone->is_group ||
            (strcmp(role, "primary") != 0 && strcmp(role, "secondary") != 0)) {
            (void)sqlite3_finalize(statement);
            return false;
        }
        ...
        if (!unclassified_only && (!primary_only || strcmp(role, "primary") == 0) &&
...
TrainlogStatus trainlog_database_list_exercises_filtered(TrainlogDatabase *database,
    ...
    static const char *const SQL =
        ...
        "FROM exercises e WHERE NOT EXISTS(SELECT 1 FROM sync_causal_state s WHERE "
        "s.target_kind='exercise' AND s.target_id=e.exercise_id AND s.deleted=1) ORDER BY name "
        "COLLATE NOCASE,exercise_id;";
    sqlite3_stmt *statement = NULL;
    size_t count = 0U;
    size_t prefix_length;
    int rc;
    if (database == NULL || normalized_prefix == NULL || output_count == NULL ||
        (capacity > 0U && output == NULL) || (unclassified_only && zone_id != NULL) ||
    ...
    *output_count = 0U;
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        if (!matches && !exercise_row_matches_body_zone(database,
                                                        row_id,
        ...
        if (count < capacity) {
            ...
            (void)memset(item, 0, sizeof(*item));
            (void)snprintf(item->exercise_id,
                           sizeof(item->exercise_id),
                           "%s",
            ...
            item->tracking_mode =
                tracking_mode_from_sql((const char *)sqlite3_column_text(statement, 3));
            item->recording_mode =
                recording_mode_from_sql((const char *)sqlite3_column_text(statement, 4));
...
static TrainlogStatus lookup_session_row_id(TrainlogDatabase *database,
                                            const char *session_id,
                                            sqlite3_int64 *output_row_id) {
    sqlite3_stmt *statement = NULL;
    int rc;

    if (database == NULL || database->connection == NULL || session_id == NULL ||
        session_id[0] == '\0' || output_row_id == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    rc = sqlite3_bind_text(statement, 1, session_id, -1, SQLITE_TRANSIENT);
    if (rc != SQLITE_OK) {
        (void)sqlite3_finalize(statement);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
...
static TrainlogStatus insert_session_header(TrainlogDatabase *database,
    ...
    rc = sqlite3_bind_text(statement, 1, session->session_id, -1, SQLITE_TRANSIENT);

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 2, session->started_at, -1, SQLITE_TRANSIENT);
    }

    if (rc == SQLITE_OK) {
        rc = session->ended_at[0] != '\0'
                 ? sqlite3_bind_text(statement, 3, session->ended_at, -1, SQLITE_TRANSIENT)
...
static TrainlogStatus insert_session_exercise(TrainlogDatabase *database,
    ...
    if (input == NULL || output_row_id == NULL ||
        (input->data_fields & ~TRAINLOG_EXERCISE_DATA_KNOWN_MASK) != 0U) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (input->entry_id[0] == '\0') {
        status = trainlog_id_generate("sxe", generated_entry_id, sizeof(generated_entry_id));
        if (status != TRAINLOG_STATUS_OK) {
            return status;
        }
    ...
    recording_mode = recording_mode_to_sql(input->recording_mode);

    tracking_mode = tracking_mode_to_sql(input->tracking_mode);

    if (load_mode == NULL || recording_mode == NULL || tracking_mode == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }

    if (input->has_max_weight) {
        if (!isfinite(input->max_weight_kg) || input->max_weight_kg <= 0.0 ||
            input->rest_seconds != 0 || input->target_sets != 0 || input->target_reps != 0 ||
            input->target_duration_seconds != 0 || input->target_has_weight ||
            input->set_count != 0U || input->continuous_duration_seconds != 0 ||
            input->continuous_has_speed || input->continuous_has_distance) {
            return TRAINLOG_STATUS_INVALID_ARGUMENT;
        }
    } else if (input->recording_mode == TRAINLOG_RECORDING_CONTINUOUS) {
        bool speed_required = (input->data_fields & TRAINLOG_EXERCISE_DATA_SPEED_KMH) != 0U;

        bool distance_required = (input->data_fields & TRAINLOG_EXERCISE_DATA_DISTANCE_KM) != 0U;

        if (input->load_mode != TRAINLOG_LOAD_NONE || input->rest_seconds != 0 ||
            input->target_sets != 0 || input->target_reps != 0 ||
            input->target_duration_seconds != 0 || input->target_has_weight ||
            input->set_count != 0U || input->continuous_duration_seconds <= 0 ||
            input->continuous_has_speed != speed_required ||
            input->continuous_has_distance != distance_required ||
            (input->continuous_has_speed && input->continuous_speed_kmh <= 0.0) ||
            (input->continuous_has_distance && input->continuous_distance_km <= 0.0)) {
            return TRAINLOG_STATUS_INVALID_ARGUMENT;
        }
    } else if (input->recording_mode == TRAINLOG_RECORDING_SETS) {
        bool has_target_sets = input->target_sets > 0;

        bool has_target_reps = input->target_reps > 0;

        bool has_target_duration = input->target_duration_seconds > 0;

        bool any_target = has_target_sets || has_target_reps || has_target_duration;

        bool target_valid =
            !any_target || (has_target_sets && (has_target_reps != has_target_duration));

        if (input->target_sets < 0 || input->target_reps < 0 ||
            input->target_duration_seconds < 0 || !target_valid ||
            input->continuous_duration_seconds != 0 || input->continuous_has_speed ||
    ...
    status = lookup_exercise_row_id(database, input->exercise_id, &exercise_row_id);
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 2, session_row_id);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 3, exercise_row_id);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 4, recording_mode, -1, SQLITE_STATIC);
    }
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 6, (sqlite3_int64)input->data_fields);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 7, (sqlite3_int64)position);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 8, load_mode, -1, SQLITE_STATIC);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int(statement, 9, input->rest_seconds);
    }

    if (rc == SQLITE_OK) {
        rc = input->target_sets > 0 ? sqlite3_bind_int(statement, 10, input->target_sets)
                                    : sqlite3_bind_null(statement, 10);
    }

    if (rc == SQLITE_OK) {
        rc = input->target_reps > 0 ? sqlite3_bind_int(statement, 11, input->target_reps)
                                    : sqlite3_bind_null(statement, 11);
    }

    if (rc == SQLITE_OK) {
        rc = input->target_duration_seconds > 0
                 ? sqlite3_bind_int(statement, 12, input->target_duration_seconds)
                 : sqlite3_bind_null(statement, 12);
    }

    if (rc == SQLITE_OK) {
        rc = input->target_has_weight ? sqlite3_bind_double(statement, 13, input->target_weight_kg)
                                      : sqlite3_bind_null(statement, 13);
    }

    if (rc == SQLITE_OK) {
        rc = input->notes != NULL && input->notes[0] != '\0'
                 ? sqlite3_bind_text(statement, 14, input->notes, -1, SQLITE_TRANSIENT)
                 : sqlite3_bind_null(statement, 14);
    }

    if (rc == SQLITE_OK) {
        rc = input->equipment_id[0] != '\0'
                 ? sqlite3_bind_text(statement, 15, input->equipment_id, -1, SQLITE_TRANSIENT)
...
static TrainlogStatus insert_continuous_activity(TrainlogDatabase *database,
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int(statement, 2, input->continuous_duration_seconds);
    }

    if (rc == SQLITE_OK) {
        rc = input->continuous_has_speed
                 ? sqlite3_bind_double(statement, 3, input->continuous_speed_kmh)
                 : sqlite3_bind_null(statement, 3);
    }

    if (rc == SQLITE_OK) {
        rc = input->continuous_has_distance
                 ? sqlite3_bind_double(statement, 4, input->continuous_distance_km)
...
static TrainlogStatus insert_performed_set(TrainlogDatabase *database,
                                           sqlite3_int64 session_exercise_row_id,
                                           size_t position,
    ...
    if (database == NULL || database->connection == NULL || input == NULL ||
        ...
        (input->has_weight && (!isfinite(input->weight_kg) || input->weight_kg < 0.0))) {
    ...
    if (rc == SQLITE_OK) {
        rc = input->duration_seconds > 0 ? sqlite3_bind_null(statement, 3)
                                         : sqlite3_bind_int(statement, 3, input->reps);
    }
    if (rc == SQLITE_OK) {
        rc = input->duration_seconds > 0 ? sqlite3_bind_int(statement, 4, input->duration_seconds)
                                         : sqlite3_bind_null(statement, 4);
    }
    if (rc == SQLITE_OK) {
        rc = input->has_weight ? sqlite3_bind_double(statement, 5, input->weight_kg)
...
static TrainlogStatus insert_max_result(TrainlogDatabase *database,
                                        sqlite3_int64 session_exercise_row_id,
                                        double max_weight_kg) {
    sqlite3_stmt *statement = NULL;
    int rc;

    if (database == NULL || database->connection == NULL || !isfinite(max_weight_kg) ||
        max_weight_kg <= 0.0) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
...
static TrainlogStatus insert_session_children(TrainlogDatabase *database,
                                              sqlite3_int64 session_row_id,
    ...
    for (exercise_index = 0U; exercise_index < exercise_count; ++exercise_index) {
        const TrainlogSessionExerciseInput *exercise = &exercises[exercise_index];

        sqlite3_int64 session_exercise_row_id;
        size_t set_index;
        TrainlogStatus status;

        if (exercise->has_max_weight && session_type != TRAINLOG_SESSION_MAX_TEST) {
            return TRAINLOG_STATUS_INVALID_ARGUMENT;
        }

        status = insert_session_exercise(
            database, session_row_id, exercise_index, exercise, &session_exercise_row_id);
        ...
        if (exercise->has_max_weight) {
            status = insert_max_result(database, session_exercise_row_id, exercise->max_weight_kg);
            if (status != TRAINLOG_STATUS_OK) {
                return status;
            }
            continue;
        ...
        if (exercise->recording_mode == TRAINLOG_RECORDING_CONTINUOUS) {
            status = insert_continuous_activity(database, session_exercise_row_id, exercise);
        ...
        for (set_index = 0U; set_index < exercise->set_count; ++set_index) {
            ...
            status = insert_performed_set(
                database, session_exercise_row_id, set_index, &exercise->sets[set_index]);
...
static TrainlogStatus validate_actual_set_weights(const TrainlogSessionExerciseInput *exercises,
                                                  size_t exercise_count) {
    size_t exercise_index;

    if ((exercise_count > 0U && exercises == NULL) || exercise_count > (size_t)INT64_MAX) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    for (exercise_index = 0U; exercise_index < exercise_count; ++exercise_index) {
        ...
        for (set_index = 0U; set_index < exercise->set_count; ++set_index) {
            ...
            if (set->has_weight && (!isfinite(set->weight_kg) || set->weight_kg < 0.0)) {
                return TRAINLOG_STATUS_INVALID_ARGUMENT;
            }
        }
    }
...
TrainlogStatus trainlog_database_insert_session(TrainlogDatabase *database,
                                                const TrainlogSessionInput *session) {
    sqlite3_int64 session_row_id;
    TrainlogStatus status;

    if (database == NULL || database->connection == NULL || session == NULL ||
        session->session_id[0] == '\0' || session->started_at[0] == '\0' ||
        (session->exercise_count > 0U && session->exercises == NULL)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }

    status = validate_actual_set_weights(session->exercises, session->exercise_count);
    if (status != TRAINLOG_STATUS_OK) {
        return status;
    }
    ...
    if (status == TRAINLOG_STATUS_OK) {
        status = insert_session_children(database,
                                         session_row_id,
...
TrainlogStatus
trainlog_database_replace_session_exercises(TrainlogDatabase *database,
                                            const char *session_id,
                                            const TrainlogSessionExerciseInput *exercises,
                                            size_t exercise_count) {
    sqlite3_int64 session_row_id;
    ...
    if (database == NULL || database->connection == NULL || session_id == NULL ||
        session_id[0] == '\0' || (exercise_count > 0U && exercises == NULL)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    status = lookup_session_row_id(database, session_id, &session_row_id);
    ...
    if (rc != SQLITE_ROW ||
        !session_type_from_sql((const char *)sqlite3_column_text(statement, 0), &session_type)) {
        (void)sqlite3_finalize(statement);
        (void)trainlog_database_rollback(database);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    ...
    rc = sqlite3_exec(database->connection,
    ...
    if (statement != NULL && sqlite3_finalize(statement) != SQLITE_OK && rc == SQLITE_DONE) {
        rc = SQLITE_ERROR;
    }
    statement = NULL;
    if (rc != SQLITE_DONE) {
    ...
    status =
        insert_session_children(database, session_row_id, session_type, exercises, exercise_count);
    ...
    if (rc != SQLITE_OK || sqlite3_exec(database->connection,
     ...
     * cannot be modified or silently reactivated by this writer. */
    {
        ...
        rc = sqlite3_prepare_v2(
            database->connection,
            "INSERT INTO "
            "sync_causal_state(target_kind,target_id,current_revision_id,deleted,operation_id) "
            "VALUES('session',?1,'mu_'||lower(hex(randomblob(16))),0,NULL) "
            "ON CONFLICT(target_kind,target_id) DO UPDATE SET "
            "current_revision_id=excluded.current_revision_id "
            "WHERE sync_causal_state.deleted=0;",
            -1,
            &causal,
            NULL);
        if (rc == SQLITE_OK) {
            rc = sqlite3_bind_text(causal, 1, session_id, -1, SQLITE_TRANSIENT);
        }
        if (rc == SQLITE_OK) {
            rc = sqlite3_step(causal);
        }
        if (causal != NULL && sqlite3_finalize(causal) != SQLITE_OK && rc == SQLITE_DONE) {
            rc = SQLITE_ERROR;
        }
...
TrainlogStatus trainlog_database_list_sessions(TrainlogDatabase *database,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (count < capacity) {
            ...
            const unsigned char *session_type = sqlite3_column_text(statement, 3);

            sqlite3_int64 exercise_count = sqlite3_column_int64(statement, 4);

            if (id == NULL || started == NULL || ended == NULL || session_type == NULL ||
                exercise_count < 0) {
                (void)sqlite3_finalize(statement);
                return TRAINLOG_STATUS_DATABASE_ERROR;
            }
            ...
            (void)snprintf(
                output[count].session_id, sizeof(output[count].session_id), "%s", (const char *)id);

            (void)snprintf(output[count].started_at,
                           sizeof(output[count].started_at),
                           "%s",
                           (const char *)started);

            (void)snprintf(
                output[count].ended_at, sizeof(output[count].ended_at), "%s", (const char *)ended);

            if (!session_type_from_sql((const char *)session_type, &output[count].session_type)) {
                (void)sqlite3_finalize(statement);
                return TRAINLOG_STATUS_DATABASE_ERROR;
            }
...
static int bind_optional_double(sqlite3_stmt *statement, int index, bool present, double value) {
    return present ? sqlite3_bind_double(statement, index, value)
                   : sqlite3_bind_null(statement, index);
}

TrainlogStatus
trainlog_database_insert_body_observation(TrainlogDatabase *database,
    ...
    if (database == NULL || database->connection == NULL || observation == NULL ||
        observation->observation_id[0] == '\0' || observation->observed_at[0] == '\0') {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }

    any_metric = observation->has_body_weight || observation->has_neck ||
                 observation->has_shoulders || observation->has_chest || observation->has_waist ||
                 observation->has_hips || observation->has_left_arm || observation->has_right_arm ||
                 observation->has_left_forearm || observation->has_right_forearm ||
                 observation->has_left_thigh || observation->has_right_thigh ||
                 observation->has_left_calf || observation->has_right_calf;
    ...
    rc = sqlite3_bind_text(statement, 1, observation->observation_id, -1, SQLITE_TRANSIENT);
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 2, observation->observed_at, -1, SQLITE_TRANSIENT);
    }

    if (rc == SQLITE_OK) {
        rc = observation->session_id != NULL && observation->session_id[0] != '\0'
                 ? sqlite3_bind_text(statement, 3, observation->session_id, -1, SQLITE_TRANSIENT)
                 : sqlite3_bind_null(statement, 3);
    }
...
#define BIND_METRIC(index_, flag_, value_)                                                         \
    do {                                                                                           \
        if (rc == SQLITE_OK) {                                                                     \
            rc = bind_optional_double(statement, (index_), (flag_), (value_));                     \
        }                                                                                          \
    } while (0)
    ...
    BIND_METRIC(12, observation->has_left_forearm, observation->left_forearm_cm);
    BIND_METRIC(13, observation->has_right_forearm, observation->right_forearm_cm);
...
#undef BIND_METRIC
    ...
    if (rc == SQLITE_OK) {
        rc = observation->notes != NULL && observation->notes[0] != '\0'
                 ? sqlite3_bind_text(statement, 18, observation->notes, -1, SQLITE_TRANSIENT)
...
TrainlogStatus trainlog_database_list_weight_points(TrainlogDatabase *database,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (count < capacity) {
            ...
            (void)snprintf(output[count].observed_at,
                           sizeof(output[count].observed_at),
                           "%s",
                           (const char *)observed);
            output[count].body_weight_kg = sqlite3_column_double(statement, 1);
        }
        ++count;
    }
...
static TrainlogStatus
detail_append_text(char *output, size_t output_size, size_t *used, const char *text) {
    size_t remaining;
    int written;

    if (output == NULL || used == NULL || text == NULL || *used >= output_size) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }

    remaining = output_size - *used;

    written = snprintf(output + *used, remaining, "%s", text);
...
static TrainlogStatus detail_fill_sets(TrainlogDatabase *database,
                                       sqlite3_int64 session_exercise_row_id,
                                       TrainlogPersistedExerciseDetail *detail) {
    static const char *const SQL = "SELECT reps, duration_seconds, weight_kg "
                                   "FROM performed_sets "
    ...
    rc = sqlite3_prepare_v2(database->connection,
                            ...
                            NULL);
    if (rc != SQLITE_OK ||
        sqlite3_bind_int64(count_statement, 1, session_exercise_row_id) != SQLITE_OK ||
    ...
    }
    {
        ...
        if (persisted_count < 0 ||
            (uint64_t)persisted_count > (uint64_t)(SIZE_MAX / sizeof(*detail->actual_sets))) {
            (void)sqlite3_finalize(count_statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        char fragment[96];
        int written;
        int has_reps = sqlite3_column_type(statement, 0) != SQLITE_NULL;
        int has_duration = sqlite3_column_type(statement, 1) != SQLITE_NULL;
        int has_weight = sqlite3_column_type(statement, 2) != SQLITE_NULL;
        ...
        detail->actual_sets[count].reps = has_reps != 0 ? sqlite3_column_int(statement, 0) : 0;
        detail->actual_sets[count].duration_seconds =
            has_duration != 0 ? sqlite3_column_int(statement, 1) : 0;
        detail->actual_sets[count].has_weight = has_weight != 0;
        detail->actual_sets[count].weight_kg =
            has_weight != 0 ? sqlite3_column_double(statement, 2) : 0.0;

        if (count > 0U) {
            TrainlogStatus status = detail_append_text(
                detail->actual_summary, sizeof(detail->actual_summary), &used, " / ");
        ...
        if (has_reps != 0) {
            ...
            if (has_weight != 0) {
                written = snprintf(fragment,
                                   sizeof(fragment),
                                   "%d@%.1f",
                                   reps,
                                   sqlite3_column_double(statement, 2));
            } else {
                written = snprintf(fragment, sizeof(fragment), "%d", reps);
            }
            ...
            if (trainlog_duration_format(duration, duration_text, sizeof(duration_text)) !=
                TRAINLOG_STATUS_OK) {
                (void)sqlite3_finalize(statement);
                free(detail->actual_sets);
                detail->actual_sets = NULL;
            ...
            if (has_weight != 0) {
                ...
                written = snprintf(fragment, sizeof(fragment), "%s", duration_text);
            }
        } else {
            (void)sqlite3_finalize(statement);
            free(detail->actual_sets);
        ...
        {
            TrainlogStatus status = detail_append_text(
                detail->actual_summary, sizeof(detail->actual_summary), &used, fragment);
    ...
    if (count == 0U) {
        (void)snprintf(
            detail->actual_summary, sizeof(detail->actual_summary), "%s", "aucune série réalisée");
    }

    return TRAINLOG_STATUS_OK;
}

void trainlog_database_free_session_details(TrainlogPersistedExerciseDetail *exercises,
                                            size_t exercise_count) {
    size_t index;
...
TrainlogStatus
trainlog_database_get_session_details(TrainlogDatabase *database,
                                      const char *session_id,
    ...
    static const char *const EXERCISE_SQL = "SELECT "
    ...
    if (database == NULL || database->connection == NULL || session_id == NULL ||
        session_id[0] == '\0' || output_session == NULL || output_exercise_count == NULL ||
    ...
    rc = sqlite3_prepare_v2(database->connection, HEADER_SQL, -1, &header, NULL);

    if (rc != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }

    rc = sqlite3_bind_text(header, 1, session_id, -1, SQLITE_TRANSIENT);
    ...
    {
        const unsigned char *started = sqlite3_column_text(header, 0);

        const unsigned char *ended = sqlite3_column_text(header, 1);

        const unsigned char *session_type = sqlite3_column_text(header, 2);

        if (started == NULL || ended == NULL || session_type == NULL) {
            (void)sqlite3_finalize(header);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }

        (void)snprintf(
            output_session->session_id, sizeof(output_session->session_id), "%s", session_id);

        (void)snprintf(output_session->started_at,
                       sizeof(output_session->started_at),
                       "%s",
                       (const char *)started);

        (void)snprintf(
            output_session->ended_at, sizeof(output_session->ended_at), "%s", (const char *)ended);

        if (!session_type_from_sql((const char *)session_type, &output_session->session_type)) {
            (void)sqlite3_finalize(header);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
    }
    ...
    rc = sqlite3_prepare_v2(database->connection, EXERCISE_SQL, -1, &exercises, NULL);

    if (rc != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }

    rc = sqlite3_bind_text(exercises, 1, session_id, -1, SQLITE_TRANSIENT);
    ...
    while ((rc = sqlite3_step(exercises)) == SQLITE_ROW) {
        if (copied < exercise_capacity) {
            TrainlogPersistedExerciseDetail *detail = &output_exercises[copied];

            const unsigned char *name = sqlite3_column_text(exercises, 0);

            const unsigned char *tracking = sqlite3_column_text(exercises, 1);

            const unsigned char *recording = sqlite3_column_text(exercises, 2);

            sqlite3_int64 data_fields = sqlite3_column_int64(exercises, 3);

            const unsigned char *load = sqlite3_column_text(exercises, 4);

            sqlite3_int64 session_exercise_row_id = sqlite3_column_int64(exercises, 16);
            const unsigned char *equipment_id = sqlite3_column_text(exercises, 14);
            const unsigned char *entry_id = sqlite3_column_text(exercises, 15);

            TrainlogStatus status;

            if (name == NULL || tracking == NULL || recording == NULL || load == NULL ||
                data_fields < 0 || (uint64_t)data_fields > (uint64_t)UINT32_MAX ||
                (((TrainlogExerciseDataFields)data_fields) & ~TRAINLOG_EXERCISE_DATA_KNOWN_MASK) !=
                    0U) {
                (void)sqlite3_finalize(exercises);
            ...
            (void)snprintf(
                detail->entry_id, sizeof(detail->entry_id), "%s", (const char *)entry_id);

            (void)snprintf(detail->name, sizeof(detail->name), "%s", (const char *)name);

            detail->tracking_mode = strcmp((const char *)tracking, "duration") == 0
                                        ? TRAINLOG_TRACKING_DURATION
                                        : TRAINLOG_TRACKING_REPS;

            detail->recording_mode = recording_mode_from_sql((const char *)recording);

            detail->data_fields = (TrainlogExerciseDataFields)data_fields;

            detail->load_mode = detail_load_mode_from_text((const char *)load);

            detail->rest_seconds = sqlite3_column_int(exercises, 5);

            detail->target_sets = sqlite3_column_int(exercises, 6);

            detail->target_reps = sqlite3_column_int(exercises, 7);

            detail->target_duration_seconds = sqlite3_column_int(exercises, 8);

            detail->has_target_weight = sqlite3_column_type(exercises, 9) != SQLITE_NULL;

            detail->target_weight_kg =
                detail->has_target_weight != 0 ? sqlite3_column_double(exercises, 9) : 0.0;

            detail->has_max_weight = sqlite3_column_type(exercises, 13) != SQLITE_NULL;
            detail->max_weight_kg =
                detail->has_max_weight != 0 ? sqlite3_column_double(exercises, 13) : 0.0;

            if (detail->has_max_weight != 0) {
                detail->actual_set_count = 0U;
                (void)snprintf(detail->actual_summary,
                               sizeof(detail->actual_summary),
                               "Max %.2f kg",
                               detail->max_weight_kg);
            } else if (detail->recording_mode == TRAINLOG_RECORDING_CONTINUOUS) {
                bool speed_required =
                    (detail->data_fields & TRAINLOG_EXERCISE_DATA_SPEED_KMH) != 0U;

                bool distance_required =
                    (detail->data_fields & TRAINLOG_EXERCISE_DATA_DISTANCE_KM) != 0U;

                if (sqlite3_column_type(exercises, 10) == SQLITE_NULL) {
                    (void)sqlite3_finalize(exercises);
                    trainlog_database_free_session_details(output_exercises, copied);
                    return TRAINLOG_STATUS_DATABASE_ERROR;
                }

                detail->continuous_duration_seconds = sqlite3_column_int(exercises, 10);

                detail->has_continuous_speed = sqlite3_column_type(exercises, 11) != SQLITE_NULL;

                detail->continuous_speed_kmh =
                    detail->has_continuous_speed != 0 ? sqlite3_column_double(exercises, 11) : 0.0;

                detail->has_continuous_distance = sqlite3_column_type(exercises, 12) != SQLITE_NULL;

                detail->continuous_distance_km = detail->has_continuous_distance != 0
                                                     ? sqlite3_column_double(exercises, 12)
                                                     : 0.0;

                if ((detail->has_continuous_speed != 0) != speed_required ||
                    (detail->has_continuous_distance != 0) != distance_required) {
                    (void)sqlite3_finalize(exercises);
                    trainlog_database_free_session_details(output_exercises, copied);
                    return TRAINLOG_STATUS_DATABASE_ERROR;
                }
                ...
                status = detail_fill_sets(database, session_exercise_row_id, detail);
...
TrainlogStatus trainlog_database_list_body_metric_points(TrainlogDatabase *database,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (count < capacity) {
            ...
            output[count].value = sqlite3_column_double(statement, 1);
        }
...
TrainlogStatus trainlog_database_latest_body_pair(TrainlogDatabase *database,
...

    {
        ...
        (void)snprintf(
            output->observed_at, sizeof(output->observed_at), "%s", (const char *)observed);

        output->left_value = sqlite3_column_double(statement, 1);

        output->right_value = sqlite3_column_double(statement, 2);
    }
...
static TrainlogLoadMode performance_load_mode_from_sql(const char *text) {
    if (text != NULL && strcmp(text, "external") == 0) {
        return TRAINLOG_LOAD_EXTERNAL;
    }
...
static bool performance_candidate_better(const TrainlogExercisePerformancePoint *current,
                                         TrainlogLoadMode load_mode,
                                         int metric_value,
                                         int has_weight,
                                         double weight_kg) {
    if (current == NULL || metric_value <= 0) {
        return false;
    }
    ...
    case TRAINLOG_LOAD_EXTERNAL:
        ...
        return weight_kg == current->weight_kg && metric_value > current->metric_value;
...
TrainlogStatus trainlog_database_list_exercise_performance(TrainlogDatabase *database,
                                                           const char *exercise_id,
    ...
    if (database == NULL || database->connection == NULL || exercise_id == NULL ||
        exercise_id[0] == '\0' || output_count == NULL || (capacity > 0U && output == NULL)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    *output_count = 0U;
    ...
    rc = sqlite3_bind_text(statement, 1, exercise_id, -1, SQLITE_TRANSIENT);
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        const unsigned char *session_id = sqlite3_column_text(statement, 0);

        const unsigned char *started_at = sqlite3_column_text(statement, 1);

        const unsigned char *session_type = sqlite3_column_text(statement, 2);

        const unsigned char *tracking_mode = sqlite3_column_text(statement, 3);

        const unsigned char *load_mode = sqlite3_column_text(statement, 4);

        bool new_session;
        const unsigned char *equipment_id = sqlite3_column_text(statement, 10);

        if (session_id == NULL || started_at == NULL || session_type == NULL ||
            tracking_mode == NULL || load_mode == NULL || equipment_id == NULL ||
        ...
        new_session = current_session_id[0] == '\0' ||
                      strcmp(current_session_id, (const char *)session_id) != 0;

        if (new_session) {
            (void)snprintf(
                current_session_id, sizeof(current_session_id), "%s", (const char *)session_id);
            ...
            if (copied < capacity) {
                ...
                (void)snprintf(current->session_id,
                               sizeof(current->session_id),
                               "%s",
                               (const char *)session_id);

                (void)snprintf(current->started_at,
                               sizeof(current->started_at),
                               "%s",
                               (const char *)started_at);

                if (!session_type_from_sql((const char *)session_type, &current->session_type)) {
                    (void)sqlite3_finalize(statement);

                    return TRAINLOG_STATUS_DATABASE_ERROR;
                }

                current->tracking_mode = tracking_mode_from_sql((const char *)tracking_mode);

                current->load_mode = performance_load_mode_from_sql((const char *)load_mode);

                ++copied;
            }
        }

        if (current != NULL && sqlite3_column_type(statement, 9) != SQLITE_NULL) {
            ...
            if (current->has_performance == 0 || max_weight > current->weight_kg) {
                ...
                (void)snprintf(current->equipment_id,
                               sizeof(current->equipment_id),
                               "%s",
                               (const char *)equipment_id);
            }
        } else if (current != NULL && sqlite3_column_type(statement, 5) != SQLITE_NULL) {
            int metric_value;
            int has_weight;
            double weight_kg;
            ...
            if (current->tracking_mode == TRAINLOG_TRACKING_REPS) {
                metric_value = sqlite3_column_type(statement, 6) != SQLITE_NULL
                                   ? sqlite3_column_int(statement, 6)
                                   : 0;
            } else {
                metric_value = sqlite3_column_type(statement, 7) != SQLITE_NULL
                                   ? sqlite3_column_int(statement, 7)
                                   : 0;
            }

            has_weight = sqlite3_column_type(statement, 8) != SQLITE_NULL;

            weight_kg = has_weight != 0 ? sqlite3_column_double(statement, 8) : 0.0;

            if (performance_candidate_better(
                    current, current->load_mode, metric_value, has_weight, weight_kg)) {
                current->has_performance = 1;
...
TrainlogStatus
trainlog_database_load_session_editable(TrainlogDatabase *database,
                                        const char *session_id,
                                        TrainlogSessionSummary *output_session,
    ...
    static const char *const HEADER_SQL = "SELECT "
                                          "started_at, "
                                          "COALESCE(ended_at, ''), "
                                          "session_type "
                                          "FROM sessions "
    ...
    static const char *const SET_SQL = "SELECT reps, duration_seconds, weight_kg "
                                       "FROM performed_sets "
                                       "WHERE session_exercise_row_id = ?1 "
                                       "ORDER BY position ASC;";
    ...
    if (database == NULL || database->connection == NULL || session_id == NULL ||
        session_id[0] == '\0' || output_session == NULL || output_exercise_count == NULL ||
        output_set_count == NULL || (exercise_capacity > 0U && output_exercises == NULL) ||
        (set_capacity > 0U && output_sets == NULL)) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    *output_set_count = 0U;
    ...
    rc = sqlite3_prepare_v2(database->connection, EXERCISE_SQL, -1, &exercise_statement, NULL);

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(exercise_statement, 1, session_id, -1, SQLITE_TRANSIENT);
    }
    ...
    while ((rc = sqlite3_step(exercise_statement)) == SQLITE_ROW) {
        ...
        record = &output_exercises[exercise_count];

        session_exercise_row_id = sqlite3_column_int64(exercise_statement, 0);

        exercise_id = sqlite3_column_text(exercise_statement, 1);

        name = sqlite3_column_text(exercise_statement, 2);

        tracking = sqlite3_column_text(exercise_statement, 3);

        load = sqlite3_column_text(exercise_statement, 4);

        notes = sqlite3_column_text(exercise_statement, 10);

        note_bytes = sqlite3_column_bytes(exercise_statement, 10);

        if (exercise_id == NULL || name == NULL || tracking == NULL || load == NULL ||
            notes == NULL || note_bytes < 0 || (size_t)note_bytes > TRAINLOG_NOTE_MAX) {
            (void)sqlite3_finalize(exercise_statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
        ...
        if (sqlite3_column_type(exercise_statement, 11) != SQLITE_NULL) {
            ...
            (void)snprintf(record->equipment_id,
                           sizeof(record->equipment_id),
                           "%s",
                           (const char *)equipment_id);
        }
        ...
        (void)snprintf(record->entry_id,
                       sizeof(record->entry_id),
                       "%s",
                       (const char *)sqlite3_column_text(exercise_statement, 12));

        (void)snprintf(
            record->exercise_id, sizeof(record->exercise_id), "%s", (const char *)exercise_id);

        (void)snprintf(record->name, sizeof(record->name), "%s", (const char *)name);

        record->tracking_mode = tracking_mode_from_sql((const char *)tracking);

        record->recording_mode =
            recording_mode_from_sql((const char *)sqlite3_column_text(exercise_statement, 14));
        {
            sqlite3_int64 data_fields = sqlite3_column_int64(exercise_statement, 15);
            if (data_fields < 0 || (uint64_t)data_fields > (uint64_t)UINT32_MAX ||
                (((TrainlogExerciseDataFields)data_fields) & ~TRAINLOG_EXERCISE_DATA_KNOWN_MASK) !=
                    0U) {
                (void)sqlite3_finalize(exercise_statement);
                return TRAINLOG_STATUS_DATABASE_ERROR;
            }
            record->data_fields = (TrainlogExerciseDataFields)data_fields;
        }

        record->load_mode = detail_load_mode_from_text((const char *)load);

        record->rest_seconds = sqlite3_column_int(exercise_statement, 5);

        record->target_sets = sqlite3_column_int(exercise_statement, 6);

        record->target_reps = sqlite3_column_int(exercise_statement, 7);

        record->target_duration_seconds = sqlite3_column_int(exercise_statement, 8);

        record->has_target_weight = sqlite3_column_type(exercise_statement, 9) != SQLITE_NULL;

        record->target_weight_kg =
            record->has_target_weight != 0 ? sqlite3_column_double(exercise_statement, 9) : 0.0;

        record->has_max_weight = sqlite3_column_type(exercise_statement, 13) != SQLITE_NULL;
        record->max_weight_kg =
            record->has_max_weight != 0 ? sqlite3_column_double(exercise_statement, 13) : 0.0;
        ...
        if (sqlite3_column_type(exercise_statement, 16) != SQLITE_NULL) {
            record->continuous_duration_seconds = sqlite3_column_int(exercise_statement, 16);
            record->has_continuous_speed =
                sqlite3_column_type(exercise_statement, 17) != SQLITE_NULL;
            record->continuous_speed_kmh = record->has_continuous_speed != 0
            ...
            record->continuous_distance_km = record->has_continuous_distance != 0
                                                 ? sqlite3_column_double(exercise_statement, 18)
                                                 : 0.0;
        }

        (void)snprintf(record->notes, sizeof(record->notes), "%s", (const char *)notes);

        record->set_offset = set_count;

        rc = sqlite3_prepare_v2(database->connection, SET_SQL, -1, &set_statement, NULL);

        if (rc == SQLITE_OK) {
            rc = sqlite3_bind_int64(set_statement, 1, session_exercise_row_id);
        }
        ...
        while ((rc = sqlite3_step(set_statement)) == SQLITE_ROW) {
            ...
            has_reps = sqlite3_column_type(set_statement, 0) != SQLITE_NULL;

            has_duration = sqlite3_column_type(set_statement, 1) != SQLITE_NULL;
            ...
            if (has_reps != 0) {
                set->reps = sqlite3_column_int(set_statement, 0);
            } else {
                set->duration_seconds = sqlite3_column_int(set_statement, 1);
            }

            set->has_weight = sqlite3_column_type(set_statement, 2) != SQLITE_NULL;

            set->weight_kg = set->has_weight ? sqlite3_column_double(set_statement, 2) : 0.0;
    ...
    output_session->exercise_count = exercise_count;

    *output_exercise_count = exercise_count;
...
static bool body_record_fill(sqlite3_stmt *statement, TrainlogBodyObservationRecord *record) {
    ...
    if (statement == NULL || record == NULL) {
        return false;
    }

    observation_id = sqlite3_column_text(statement, 0);

    observed_at = sqlite3_column_text(statement, 1);

    session_id = sqlite3_column_text(statement, 2);

    notes = sqlite3_column_text(statement, 17);

    note_bytes = sqlite3_column_bytes(statement, 17);

    if (observation_id == NULL || observed_at == NULL || session_id == NULL || notes == NULL ||
        note_bytes < 0 || (size_t)note_bytes > TRAINLOG_NOTE_MAX) {
        return false;
    }
    ...
    (void)snprintf(
        record->observation_id, sizeof(record->observation_id), "%s", (const char *)observation_id);

    (void)snprintf(
        record->observed_at, sizeof(record->observed_at), "%s", (const char *)observed_at);

    (void)snprintf(record->session_id, sizeof(record->session_id), "%s", (const char *)session_id);

#define READ_BODY_VALUE(column_, flag_, value_)                                                    \
    do {                                                                                           \
        (flag_) = sqlite3_column_type(statement, (column_)) != SQLITE_NULL;                        \
        (value_) = (flag_) ? sqlite3_column_double(statement, (column_)) : 0.0;                    \
    } while (0)

    READ_BODY_VALUE(3, record->has_body_weight, record->body_weight_kg);

    READ_BODY_VALUE(4, record->has_neck, record->neck_cm);

    READ_BODY_VALUE(5, record->has_shoulders, record->shoulders_cm);

    READ_BODY_VALUE(6, record->has_chest, record->chest_cm);

    READ_BODY_VALUE(7, record->has_waist, record->waist_cm);

    READ_BODY_VALUE(8, record->has_hips, record->hips_cm);

    READ_BODY_VALUE(9, record->has_left_arm, record->left_arm_cm);

    READ_BODY_VALUE(10, record->has_right_arm, record->right_arm_cm);

    READ_BODY_VALUE(11, record->has_left_forearm, record->left_forearm_cm);

    READ_BODY_VALUE(12, record->has_right_forearm, record->right_forearm_cm);

    READ_BODY_VALUE(13, record->has_left_thigh, record->left_thigh_cm);

    READ_BODY_VALUE(14, record->has_right_thigh, record->right_thigh_cm);

    READ_BODY_VALUE(15, record->has_left_calf, record->left_calf_cm);

    READ_BODY_VALUE(16, record->has_right_calf, record->right_calf_cm);
...
static const char *const BODY_RECORD_SELECT = "SELECT "
                                              "bo.observation_id, "
                                              "bo.observed_at, "
...
TrainlogStatus trainlog_database_list_body_observations(TrainlogDatabase *database,
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        if (count < capacity) {
            if (!body_record_fill(statement, &output[count])) {
                (void)sqlite3_finalize(statement);
                return TRAINLOG_STATUS_DATABASE_ERROR;
            }
        }
...
TrainlogStatus trainlog_database_get_body_observation(TrainlogDatabase *database,
    ...
    if (database == NULL || database->connection == NULL || observation_id == NULL ||
        observation_id[0] == '\0' || output == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 1, observation_id, -1, SQLITE_TRANSIENT);
    }
    ...
    if (!body_record_fill(statement, output)) {
        (void)sqlite3_finalize(statement);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
...
TrainlogStatus
trainlog_database_update_body_observation(TrainlogDatabase *database,
                                          const TrainlogBodyObservationInput *observation) {
    static const char *const SQL = "UPDATE body_observations SET "
                                   "body_weight_kg = ?1, "
                                   "neck_cm = ?2, "
                                   "shoulders_cm = ?3, "
                                   "chest_cm = ?4, "
...
#define UPDATE_BODY_VALUE(index_, flag_, value_)                                                   \
    ...
    UPDATE_BODY_VALUE(1, observation->has_body_weight, observation->body_weight_kg);

    UPDATE_BODY_VALUE(2, observation->has_neck, observation->neck_cm);

    UPDATE_BODY_VALUE(3, observation->has_shoulders, observation->shoulders_cm);

    UPDATE_BODY_VALUE(4, observation->has_chest, observation->chest_cm);

    UPDATE_BODY_VALUE(5, observation->has_waist, observation->waist_cm);

    UPDATE_BODY_VALUE(6, observation->has_hips, observation->hips_cm);

    UPDATE_BODY_VALUE(7, observation->has_left_arm, observation->left_arm_cm);

    UPDATE_BODY_VALUE(8, observation->has_right_arm, observation->right_arm_cm);

    UPDATE_BODY_VALUE(9, observation->has_left_forearm, observation->left_forearm_cm);

    UPDATE_BODY_VALUE(10, observation->has_right_forearm, observation->right_forearm_cm);

    UPDATE_BODY_VALUE(11, observation->has_left_thigh, observation->left_thigh_cm);

    UPDATE_BODY_VALUE(12, observation->has_right_thigh, observation->right_thigh_cm);

    UPDATE_BODY_VALUE(13, observation->has_left_calf, observation->left_calf_cm);

    UPDATE_BODY_VALUE(14, observation->has_right_calf, observation->right_calf_cm);

#undef UPDATE_BODY_VALUE

    if (rc == SQLITE_OK) {
        rc = observation->notes != NULL && observation->notes[0] != '\0'
                 ? sqlite3_bind_text(statement, 15, observation->notes, -1, SQLITE_TRANSIENT)
                 : sqlite3_bind_null(statement, 15);
    }

    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(statement, 16, observation->observation_id, -1, SQLITE_TRANSIENT);
    }
...
static bool
knowledge_copy_column(sqlite3_stmt *statement, int column, char *output, size_t capacity) {
...
static TrainlogStatus knowledge_copy_dynamic_column(sqlite3_stmt *statement,
                                                    int column,
                                                    char **output,
...
static int knowledge_bytes_compare(const char *left,
                                   size_t left_length,
                                   const char *right,
                                   size_t right_length) {
...
static int knowledge_temporal_candidate_compare(const KnowledgeTemporalCandidate *left,
                                                const KnowledgeTemporalCandidate *right) {
    int result = trainlog_timestamp_compare(&left->timestamp, &right->timestamp);
    if (result != 0) {
        return result;
    }
    result = knowledge_bytes_compare(
        left->session_id, left->session_id_length, right->session_id, right->session_id_length);
    if (result != 0) {
        return result;
    }
    return knowledge_bytes_compare(
        left->entry_id, left->entry_id_length, right->entry_id, right->entry_id_length);
}

static TrainlogStatus knowledge_temporal_candidate_read(sqlite3_stmt *statement,
                                                        KnowledgeTemporalCandidate *output) {
    TrainlogStatus status;
    (void)memset(output, 0, sizeof(*output));
    if (sqlite3_column_type(statement, 0) != SQLITE_INTEGER) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    status = knowledge_copy_dynamic_column(
        statement, 1, &output->session_id, &output->session_id_length);
    if (status == TRAINLOG_STATUS_OK) {
        status = knowledge_copy_dynamic_column(
            statement, 2, &output->entry_id, &output->entry_id_length);
    }
    if (status == TRAINLOG_STATUS_OK) {
        status = knowledge_copy_dynamic_column(
            statement, 3, &output->started_at, &output->started_at_length);
    }
    if (status != TRAINLOG_STATUS_OK || !trainlog_timestamp_parse(output->started_at,
                                                                  output->started_at_length,
                                                                  &output->timestamp)) {
...
static void knowledge_temporal_candidate_insert(KnowledgeTemporalCandidate *items,
                                                size_t *count,
                                                size_t capacity,
    ...
    while (position < *count &&
           knowledge_temporal_candidate_compare(candidate, &items[position]) <= 0) {
        ++position;
    }
    ...
    if (*count == capacity) {
        knowledge_temporal_candidate_release(&items[capacity - 1U]);
    } else {
        ++*count;
    }
    for (index = *count - 1U; index > position; --index) {
...
static bool knowledge_tracking_mode(const char *value, TrainlogTrackingMode *output) {
    ...
    if (strcmp(value, "reps") == 0) {
        *output = TRAINLOG_TRACKING_REPS;
        return true;
    }
    if (strcmp(value, "duration") == 0) {
        *output = TRAINLOG_TRACKING_DURATION;
        return true;
    }
    return false;
}
...
static bool knowledge_recording_mode(const char *value, TrainlogRecordingMode *output) {
    ...
    if (strcmp(value, "sets") == 0) {
        *output = TRAINLOG_RECORDING_SETS;
        return true;
    }
    if (strcmp(value, "continuous") == 0) {
        *output = TRAINLOG_RECORDING_CONTINUOUS;
        return true;
    }
    return false;
}
...
static bool knowledge_load_mode(const char *value, TrainlogLoadMode *output) {
    ...
    if (strcmp(value, "none") == 0) {
        *output = TRAINLOG_LOAD_NONE;
        return true;
    }
    if (strcmp(value, "external") == 0) {
        *output = TRAINLOG_LOAD_EXTERNAL;
        return true;
    }
    if (strcmp(value, "assistance") == 0) {
        *output = TRAINLOG_LOAD_ASSISTANCE;
        return true;
    }
    return false;
}

TrainlogStatus trainlog_database_get_exercise_profile(TrainlogDatabase *database,
                                                      const char *exercise_id,
                                                      TrainlogExercise *output) {
    sqlite3_stmt *statement = NULL;
    int rc;
    ...
    (void)memset(output, 0, sizeof(*output));
    rc = sqlite3_prepare_v2(database->connection,
                            "SELECT exercise_id,name,tracking_mode,recording_mode,data_fields FROM "
                            "exercises WHERE exercise_id=?1;",
                            -1,
                            &statement,
                            NULL);
    if (rc == SQLITE_OK) {
    ...
    if (rc != SQLITE_ROW ||
        !knowledge_copy_column(statement, 0, output->exercise_id, sizeof(output->exercise_id)) ||
    ...
    if (sqlite3_column_type(statement, 2) != SQLITE_TEXT ||
        ...
        !knowledge_tracking_mode((const char *)sqlite3_column_text(statement, 2),
                                 &output->tracking_mode) ||
        !knowledge_recording_mode((const char *)sqlite3_column_text(statement, 3),
...
TrainlogStatus
trainlog_database_list_exercise_occurrences_page(TrainlogDatabase *database,
                                                 const char *exercise_id,
    ...
    if (output_has_more != NULL) {
        *output_has_more = false;
    }
    if (database == NULL || database->connection == NULL || exercise_id == NULL ||
        exercise_id[0] == '\0' || limit == 0U || limit > TRAINLOG_OCCURRENCE_PAGE_MAX ||
        output == NULL || output_count == NULL || output_has_more == NULL || output_next == NULL ||
        (after != NULL &&
         (!knowledge_bounded_string(
              after->started_at, sizeof(after->started_at), &cursor_started_length) ||
          !knowledge_bounded_string(
              after->session_id, sizeof(after->session_id), &cursor_session_length) ||
          !knowledge_bounded_string(
              after->entry_id, sizeof(after->entry_id), &cursor_entry_length) ||
          !trainlog_timestamp_parse(
              after->started_at, cursor_started_length, &cursor.timestamp)))) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    if (after != NULL) {
        after_value = *after;
        cursor.started_at = after_value.started_at;
        cursor.started_at_length = cursor_started_length;
        cursor.session_id = after_value.session_id;
        cursor.session_id_length = cursor_session_length;
        cursor.entry_id = after_value.entry_id;
        cursor.entry_id_length = cursor_entry_length;
        if (!trainlog_timestamp_parse(
                cursor.started_at, cursor_started_length, &cursor.timestamp)) {
            return TRAINLOG_STATUS_INVALID_ARGUMENT;
        }
    }
    status = trainlog_database_read_snapshot_begin(database);
    ...
    *output_has_more = selected_count > limit;
    ...
    for (index = 0U; index < count; ++index) {
        ...
        if (rc == SQLITE_OK) {
            rc = sqlite3_bind_int64(statement, 1, selected[index].row_id);
        }
        if (rc == SQLITE_OK) {
            rc = sqlite3_step(statement);
        }
        ...
        if (!knowledge_copy_column(statement, 0, item->session_id, sizeof(item->session_id)) ||
            ...
            !session_type_from_sql((const char *)sqlite3_column_text(statement, 5),
                                   &item->session_type)) {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
        if (sqlite3_column_type(statement, 6) != SQLITE_TEXT ||
            ...
            !knowledge_tracking_mode((const char *)sqlite3_column_text(statement, 6),
                                     &item->tracking_mode) ||
            !knowledge_recording_mode((const char *)sqlite3_column_text(statement, 7),
                                      &item->recording_mode) ||
            !knowledge_load_mode((const char *)sqlite3_column_text(statement, 9),
                                 &item->load_mode)) {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
        ...
        fields = sqlite3_column_int64(statement, 8);
        sets = sqlite3_column_int64(statement, 10);
        if (fields < 0 || (sqlite3_uint64)fields > UINT32_MAX || sets < 0 ||
            (sqlite3_uint64)sets > SIZE_MAX) {
            status = TRAINLOG_STATUS_DATABASE_ERROR;
            goto done;
        }
    ...
    *output_count = count;
    if (count > 0U) {
        (void)snprintf(output_next->started_at,
                       sizeof(output_next->started_at),
                       "%s",
                       output[count - 1U].started_at);
        (void)snprintf(output_next->session_id,
                       sizeof(output_next->session_id),
                       "%s",
                       output[count - 1U].session_id);
        (void)snprintf(output_next->entry_id,
                       sizeof(output_next->entry_id),
                       "%s",
                       output[count - 1U].entry_id);
    }
    status = TRAINLOG_STATUS_OK;
done:
    (void)sqlite3_finalize(statement);
...
TrainlogStatus trainlog_database_list_occurrence_sets_page(TrainlogDatabase *database,
                                                           const char *entry_id,
                                                           int after_position,
                                                           size_t limit,
    ...
    if (database == NULL || database->connection == NULL || entry_id == NULL ||
        entry_id[0] == '\0' || after_position < -1 || limit == 0U ||
        limit > TRAINLOG_OCCURRENCE_SET_PAGE_MAX || output == NULL || output_count == NULL ||
        output_has_more == NULL || output_next_position == NULL) {
        return TRAINLOG_STATUS_INVALID_ARGUMENT;
    }
    *output_next_position = after_position;
    rc = sqlite3_prepare_v2(database->connection,
                            "SELECT 1 FROM session_exercises WHERE entry_id=?1;",
                            -1,
                            &identity,
                            NULL);
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_text(identity, 1, entry_id, -1, SQLITE_TRANSIENT);
    }
    if (rc != SQLITE_OK) {
        (void)sqlite3_finalize(identity);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    ...
    if (rc != SQLITE_ROW || sqlite3_finalize(identity) != SQLITE_OK) {
        return TRAINLOG_STATUS_DATABASE_ERROR;
    }
    rc = sqlite3_prepare_v2(
        database->connection,
        "SELECT ps.position,ps.reps,ps.duration_seconds,ps.weight_kg FROM performed_sets ps "
        "JOIN session_exercises se ON se.id=ps.session_exercise_row_id WHERE se.entry_id=?1 AND "
        "ps.position>?2 "
        "ORDER BY ps.position ASC LIMIT ?3;",
        -1,
        &statement,
        NULL);
    if (rc == SQLITE_OK) {
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int(statement, 2, after_position);
    }
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 3, (sqlite3_int64)(limit + 1U));
    }
    if (rc != SQLITE_OK) {
        (void)sqlite3_finalize(statement);
        return TRAINLOG_STATUS_DATABASE_ERROR;
    ...
    while ((rc = sqlite3_step(statement)) == SQLITE_ROW) {
        ...
        item->position = (size_t)position;
        item->has_reps = sqlite3_column_type(statement, 1) != SQLITE_NULL;
        ...
        if (item->has_weight && !knowledge_numeric_column(statement, 3)) {
            (void)sqlite3_finalize(statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
        item->weight_kg = sqlite3_column_double(statement, 3);
        if (item->has_weight && !isfinite(item->weight_kg)) {
            (void)sqlite3_finalize(statement);
            return TRAINLOG_STATUS_DATABASE_ERROR;
        }
        *output_next_position = (int)position;
        ++count;
    }
    if (rc != SQLITE_DONE && rc != SQLITE_ROW) {
        (void)sqlite3_finalize(statement);
...
static TrainlogStatus database_latest_explicit_max_context(TrainlogDatabase *database,
                                                           const char *exercise_id,
                                                           const char *equipment_id,
    ...
    static const char *const HYDRATE_SQL =
        "SELECT "
        "s.session_id,se.entry_id,s.started_at,COALESCE(se.equipment_id,''),se.load_mode,mr.max_"
        "weight_kg "
    ...
    rc = sqlite3_prepare_v2(database->connection, HYDRATE_SQL, -1, &statement, NULL);
    if (rc == SQLITE_OK) {
        rc = sqlite3_bind_int64(statement, 1, selected[0].row_id);
    }
    if (rc == SQLITE_OK) {
        rc = sqlite3_step(statement);
    }
    if (rc != SQLITE_ROW ||
        !knowledge_copy_column(statement, 0, output->session_id, sizeof(output->session_id)) ||
        ...
        !knowledge_copy_column(statement, 3, output->equipment_id, sizeof(output->equipment_id))) {
        status = TRAINLOG_STATUS_DATABASE_ERROR;
        goto done;
    }
    ...
    if (!knowledge_numeric_column(statement, 5)) {
        status = TRAINLOG_STATUS_DATABASE_ERROR;
        goto done;
    }
    output->max_weight_kg = sqlite3_column_double(statement, 5);
    if (!isfinite(output->max_weight_kg) || output->max_weight_kg <= 0.0) {
        status = TRAINLOG_STATUS_DATABASE_ERROR;
        goto done;
    }
    output->found = true;
...
TrainlogStatus trainlog_database_latest_explicit_max_context(TrainlogDatabase *database,
                                                             const char *exercise_id,
                                                             TrainlogLatestExplicitMax *output) {
...
TrainlogStatus
trainlog_database_latest_explicit_max_equipment_context(TrainlogDatabase *database,
                                                        const char *exercise_id,
                                                        const char *equipment_id,
                                                        TrainlogLatestExplicitMax *output) {
