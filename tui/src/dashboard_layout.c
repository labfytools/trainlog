...
static const TrainlogDashboardTileLayout defaults[TRAINLOG_DASHBOARD_LAYOUT_TILE_COUNT] = {
    ...
    {"activity", 5U, 0U, 7U, 4U},
    {"progression", 0U, 4U, 8U, 5U},
    {"last-session", 8U, 4U, 4U, 3U},
...
static bool overlap(const TrainlogDashboardTileLayout *a, const TrainlogDashboardTileLayout *b) {
...
trainlog_dashboard_layout_parse(const char *json, size_t size, TrainlogDashboardLayout *layout) {
    ...
    for (index = 0U; index < TRAINLOG_DASHBOARD_LAYOUT_TILE_COUNT; ++index) {
        ...
        text = yyjson_get_str(id);
        length = yyjson_get_len(id);
    ...
    yyjson_doc_free(document);
    return trainlog_dashboard_layout_validate(layout) ? TRAINLOG_DASHBOARD_LAYOUT_OK
...
bool trainlog_dashboard_layout_serialize(const TrainlogDashboardLayout *layout,
    ...
    size_t index;
    int count;
    const char *source_name = source == TRAINLOG_DASHBOARD_LAYOUT_PERSISTED ? "persisted"
...
TrainlogDashboardLayoutResult
trainlog_dashboard_layout_load(TrainlogDashboardLayout *layout,
                               TrainlogDashboardLayoutSource *source) {
    char path[4096];
    char buffer[TRAINLOG_DASHBOARD_LAYOUT_JSON_CAPACITY];
    struct stat status;
    ssize_t count;
    int fd;
...
TrainlogDashboardLayoutResult trainlog_dashboard_layout_save(const TrainlogDashboardLayout *layout,
