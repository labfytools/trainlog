...
static const char *const V7_SQL =
    "CREATE TABLE exercises(id INTEGER PRIMARY KEY,exercise_id TEXT NOT NULL UNIQUE,name TEXT NOT "
    "NULL,normalized_name TEXT NOT NULL UNIQUE,tracking_mode TEXT NOT NULL,recording_mode TEXT NOT "
    "NULL DEFAULT 'sets',data_fields INTEGER NOT NULL DEFAULT 0);"
    "CREATE TABLE sessions(id INTEGER PRIMARY KEY,session_id TEXT NOT NULL UNIQUE,started_at TEXT "
    "NOT NULL,ended_at TEXT,session_type TEXT NOT NULL DEFAULT 'training',notes TEXT);"
    "CREATE TABLE session_exercises(id INTEGER PRIMARY KEY,entry_id TEXT NOT NULL "
    "UNIQUE,session_row_id INTEGER NOT NULL REFERENCES sessions(id),exercise_row_id INTEGER NOT "
    "NULL REFERENCES exercises(id),recording_mode TEXT NOT NULL DEFAULT 'sets',data_fields INTEGER "
    "NOT NULL DEFAULT 0,position INTEGER NOT NULL,load_mode TEXT NOT NULL,rest_seconds INTEGER NOT "
    "NULL,target_sets INTEGER,target_reps INTEGER,target_duration_seconds INTEGER,target_weight_kg "
    "REAL,equipment_id TEXT,notes TEXT,UNIQUE(session_row_id,position));"
    "CREATE TABLE performed_sets(id INTEGER PRIMARY KEY,session_exercise_row_id INTEGER NOT NULL "
    "REFERENCES session_exercises(id),position INTEGER NOT NULL,reps INTEGER,duration_seconds "
    "INTEGER,weight_kg REAL);"
    "CREATE TABLE continuous_activity(id INTEGER PRIMARY KEY,session_exercise_row_id INTEGER NOT "
    "NULL UNIQUE REFERENCES session_exercises(id),duration_seconds INTEGER NOT NULL,speed_kmh "
    "REAL,distance_km REAL);"
    "CREATE TABLE body_observations(id INTEGER PRIMARY KEY,observation_id TEXT NOT NULL "
    "UNIQUE,observed_at TEXT NOT NULL,session_row_id INTEGER REFERENCES "
    "sessions(id),body_weight_kg REAL,neck_cm REAL,shoulders_cm REAL,chest_cm REAL,waist_cm "
    "REAL,hips_cm REAL,left_arm_cm REAL,right_arm_cm REAL,left_forearm_cm REAL,right_forearm_cm "
...
static bool test_custom_equipment_round_trip(void) {
    ...
    sqlite3 *raw = NULL;
    size_t count = 0U;
    char *equipment_json = NULL;
    size_t equipment_json_size = 0U;
    int version = 0;
    int fd = mkstemp(path);
    CHECK(fd >= 0);
    CHECK(close(fd) == 0);
    ...
    (void)snprintf(custom.equipment_id,
                   sizeof(custom.equipment_id),
                   "%s",
                   "eq_123e4567-e89b-42d3-a456-426614174000");
    ...
    CHECK(trainlog_database_open(path, &database) == TRAINLOG_STATUS_OK);
    CHECK(trainlog_database_list_custom_equipment(database, listed, 2U, &count) ==
          TRAINLOG_STATUS_OK);
    CHECK(count == 1U);
    ...
    (void)snprintf(
        session.started_at, sizeof(session.started_at), "%s", "2026-09-08T10:00:00+02:00");
    ...
    CHECK(trainlog_database_get_session_details(
              database, session.session_id, &summary, detail, 2U, &count) == TRAINLOG_STATUS_OK);
    CHECK(count == 1U);
    ...
    CHECK(trainlog_database_list_exercise_equipment(
              database, exercise.exercise_id, used, 2U, &count) == TRAINLOG_STATUS_OK);
    CHECK(count == 1U);
    CHECK(used[0].origin == TRAINLOG_EQUIPMENT_CUSTOM);
    CHECK(strcmp(used[0].display_name, custom.display_name) == 0);
    CHECK(trainlog_database_resolve_equipment(database, "retired_machine", &resolved) ==
          TRAINLOG_STATUS_OK);
    CHECK(resolved.origin == TRAINLOG_EQUIPMENT_UNKNOWN);
    CHECK(strstr(resolved.display_name, "retired_machine") != NULL);
    CHECK(trainlog_web_equipment_merge_json(
              database,
              "{\"canonical_id\":\"functional_trainer\",\"duplicate_id\":\"eq_123e4567-e89b-42d3-"
              "a456-426614174000\"}",
              strlen("{\"canonical_id\":\"functional_trainer\",\"duplicate_id\":\"eq_123e4567-e89b-"
                     "42d3-a456-426614174000\"}"),
              &equipment_json,
              &equipment_json_size) == TRAINLOG_STATUS_OK);
    CHECK(equipment_json != NULL && equipment_json_size > 0U);
    free(equipment_json);
    equipment_json = NULL;
    CHECK(trainlog_database_get_session_details(
              database, session.session_id, &summary, detail, 2U, &count) == TRAINLOG_STATUS_OK);
    CHECK(count == 1U);
    CHECK(strcmp(detail[0].equipment_id, "functional_trainer") == 0);
    CHECK(trainlog_database_resolve_equipment(database, custom.equipment_id, &resolved) ==
          TRAINLOG_STATUS_OK);
    CHECK(resolved.origin == TRAINLOG_EQUIPMENT_UNKNOWN);
    CHECK(trainlog_web_equipment_merge_json(
              database,
...
static bool insert_paged_equipment(TrainlogDatabase *database,
...
static bool test_custom_equipment_page_corruption(void) {
    ...
    const int invalid_lengths[] = {
        (int)sizeof(oversized), (int)sizeof(blob), (int)sizeof(embedded_nul)};
    size_t count;
    bool more;
    size_t index;
    int fd = mkstemp(path);
    ...
    CHECK(sqlite3_prepare_v2(
              raw,
              "UPDATE custom_equipment SET label_name=?1 WHERE equipment_id='eq_page_invalid';",
              -1,
              &statement,
              NULL) == SQLITE_OK);
    ...
    for (index = 0U; index < 3U; ++index) {
        if (index == 1U) {
            ...
            CHECK(sqlite3_bind_text(statement,
                                    1,
                                    (const char *)invalid_values[index],
                                    invalid_lengths[index],
                                    SQLITE_TRANSIENT) == SQLITE_OK);
        }
        ...
        count = 9U;
        more = true;
        CHECK(trainlog_database_list_custom_equipment_page(database, 0U, page, 1U, &count, &more) ==
              TRAINLOG_STATUS_DATABASE_ERROR);
        CHECK(count == 0U && !more);
        count = 9U;
        more = true;
        CHECK(trainlog_database_list_custom_equipment_page(database, 0U, page, 2U, &count, &more) ==
              TRAINLOG_STATUS_DATABASE_ERROR);
        CHECK(count == 0U && !more);
...
static bool test_custom_equipment_page_reader(void) {
    ...
    CHECK(trainlog_database_list_custom_equipment_page(
              database, 0U, page, TRAINLOG_CUSTOM_EQUIPMENT_PAGE_MAX, &count, &more) ==
          TRAINLOG_STATUS_OK);
    ...
    CHECK(trainlog_database_list_custom_equipment_page(
              database, 128U, page, TRAINLOG_CUSTOM_EQUIPMENT_PAGE_MAX, &count, &more) ==
          TRAINLOG_STATUS_OK);
    CHECK(count == 4U);
    CHECK(!more);
    ...
    CHECK(trainlog_database_list_custom_equipment_page(database, 131U, page, 1U, &count, &more) ==
          TRAINLOG_STATUS_OK);
    CHECK(count == 1U);
    CHECK(!more);
    CHECK(strcmp(page[0].equipment_id, "eq_page_129") == 0);
    CHECK(trainlog_database_list_custom_equipment_page(database, 132U, page, 1U, &count, &more) ==
          TRAINLOG_STATUS_OK);
    CHECK(count == 0U);
    CHECK(!more);
    ...
    count = 7U;
    more = true;
    CHECK(trainlog_database_list_custom_equipment_page(database, 0U, page, 0U, &count, &more) ==
          TRAINLOG_STATUS_INVALID_ARGUMENT);
    CHECK(count == 7U);
    CHECK(more);
    CHECK(trainlog_database_list_custom_equipment_page(database, 0U, NULL, 1U, &count, &more) ==
          TRAINLOG_STATUS_INVALID_ARGUMENT);
    CHECK(trainlog_database_list_custom_equipment_page(
              database, 0U, page, TRAINLOG_CUSTOM_EQUIPMENT_PAGE_MAX + 1U, &count, &more) ==
          TRAINLOG_STATUS_INVALID_ARGUMENT);
    if ((uintmax_t)SIZE_MAX > (uintmax_t)INT64_MAX) {
        count = 7U;
        more = true;
        CHECK(trainlog_database_list_custom_equipment_page(
                  database, SIZE_MAX, page, 1U, &count, &more) == TRAINLOG_STATUS_INVALID_ARGUMENT);
        CHECK(count == 0U);
        CHECK(!more);
    }
    trainlog_database_close(database);
...
int main(void) {
    if (!test_custom_equipment_round_trip()) {
        return 1;
    }
    if (!test_custom_equipment_page_corruption()) {
        return 1;
    }
    if (!test_custom_equipment_page_reader()) {
        return 1;
    }
