...
static bool test_v7_migration_sqlite_failure_has_diagnostic(void)
{
    ...
    CHECK(database == NULL);
    CHECK(strncmp(diagnostic, "migrate database to schema v15: SQLite rc=",
        strlen("migrate database to schema v15: SQLite rc=")) == 0);
...
{
    char path[] = "/tmp/trainlog-schema-v16-XXXXXX";
    char diagnostic[256];
    sqlite3 *raw = NULL;
    TrainlogDatabase *database = NULL;
    int fd = mkstemp(path);
    ...
    CHECK(close(fd) == 0);
    CHECK(sqlite3_open(path, &raw) == SQLITE_OK);
    CHECK(sqlite3_exec(raw, "PRAGMA user_version=16;", NULL, NULL, NULL) == SQLITE_OK);
    CHECK(sqlite3_close(raw) == SQLITE_OK);
    CHECK(trainlog_database_open_with_diagnostic(path, &database, diagnostic,
        sizeof(diagnostic)) == TRAINLOG_STATUS_SCHEMA_UNSUPPORTED);
    CHECK(database == NULL);
    CHECK(strstr(diagnostic, "schema version 16 is newer") != NULL);
