...
static const char *const V7_FIXTURE_SQL =
    ...
    "NULL,data_fields INTEGER NOT NULL);"
    "CREATE TABLE sessions(id INTEGER PRIMARY KEY,session_id TEXT NOT NULL UNIQUE,started_at TEXT "
    "NOT NULL,ended_at TEXT,session_type TEXT NOT NULL,notes TEXT);"
    ...
    "NULL REFERENCES exercises(id),recording_mode TEXT NOT NULL,data_fields INTEGER NOT "
    "NULL,position INTEGER NOT NULL,load_mode TEXT NOT NULL,rest_seconds INTEGER NOT "
    ...
    "INTEGER,weight_kg REAL,UNIQUE(session_exercise_row_id,position));"
    ...
    "UNIQUE,observed_at TEXT NOT NULL,session_row_id INTEGER UNIQUE REFERENCES "
    ...
    "VALUES(21,'se_fixture','2026-09-08T06:00:00Z','2026-09-08T07:00:00Z','training','fixture "
    "session');"
    "INSERT INTO session_exercises "
    "VALUES(31,'sxe_fixture_sets',21,11,'sets',0,0,'external',90,2,5,NULL,82.5,'eq_fixture_rack','"
    "sets note');"
    "INSERT INTO session_exercises "
    "VALUES(32,'sxe_fixture_run',21,12,'continuous',3,1,'none',0,NULL,NULL,NULL,NULL,NULL,'run "
    "note');"
    ...
    "INSERT INTO "
    "body_observations(id,observation_id,observed_at,session_row_id,body_weight_kg,waist_cm,notes) "
    "VALUES(61,'bo_fixture','2026-09-08T07:05:00Z',21,74.25,81.5,'body note');"
...
static bool verify_preserved_values(const char *path) {
    ...
    CHECK(
        sqlite3_prepare_v2(
            connection,
            "SELECT "
            "s.session_id,se.entry_id,se.position,se.equipment_id,ps.position,ps.reps,ps.weight_kg "
    ...
    CHECK(sqlite3_prepare_v2(connection,
                             "SELECT "
                             "se.entry_id,se.position,ca.duration_seconds,ca.speed_kmh,ca.distance_"
                             "km,bo.observation_id,bo.body_weight_kg,bo.waist_cm,bo.notes "
                             "FROM session_exercises se JOIN continuous_activity ca ON "
                             "ca.session_exercise_row_id=se.id "
                             "JOIN sessions s ON s.id=se.session_row_id JOIN body_observations bo "
                             "ON bo.session_row_id=s.id;",
                             -1,
                             &statement,
                             NULL) == SQLITE_OK);
    CHECK(sqlite3_step(statement) == SQLITE_ROW);
...
static bool test_v7_migrates_and_current_reopens(void) {
    ...
    if (trainlog_database_open_with_diagnostic(path, &database, diagnostic, sizeof(diagnostic)) !=
        TRAINLOG_STATUS_OK) {
        fprintf(stderr, "migration diagnostic: %s\n", diagnostic);
        return false;
    }
    ...
    if (trainlog_database_open_with_diagnostic(path, &database, diagnostic, sizeof(diagnostic)) !=
        TRAINLOG_STATUS_OK) {
        fprintf(stderr, "reopen diagnostic: %s\n", diagnostic);
        return false;
    }
    trainlog_database_close(database);
    CHECK(verify_preserved_values(path));
...
static bool test_v7_migration_sqlite_failure_has_diagnostic(void) {
    ...
    CHECK(strncmp(diagnostic,
                  "migrate database to schema v35: SQLite rc=",
                  strlen("migrate database to schema v35: SQLite rc=")) == 0);
...
static bool test_newer_schema_has_application_diagnostic(void) {
    char path[] = "/tmp/trainlog-schema-v17-XXXXXX";
    char diagnostic[256];
    sqlite3 *raw = NULL;
    TrainlogDatabase *database = NULL;
    int fd = mkstemp(path);
    ...
    CHECK(close(fd) == 0);
    CHECK(sqlite3_open(path, &raw) == SQLITE_OK);
    CHECK(sqlite3_exec(raw, "PRAGMA user_version=36;", NULL, NULL, NULL) == SQLITE_OK);
    CHECK(sqlite3_close(raw) == SQLITE_OK);
    CHECK(trainlog_database_open_with_diagnostic(path, &database, diagnostic, sizeof(diagnostic)) ==
          TRAINLOG_STATUS_SCHEMA_UNSUPPORTED);
    CHECK(database == NULL);
    CHECK(strstr(diagnostic, "schema version 36 is newer") != NULL);
...
int main(void) {
    if (!test_v7_migrates_and_current_reopens() ||
        ...
        !test_unrecognized_historic_schema_has_application_diagnostic()) {
        return 1;
    }
