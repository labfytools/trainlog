...
static bool test_v7_migration_sqlite_failure_has_diagnostic(void) {
    ...
    CHECK(strncmp(diagnostic,
                  "migrate database to schema v34: SQLite rc=",
                  strlen("migrate database to schema v34: SQLite rc=")) == 0);
...
static bool test_newer_schema_has_application_diagnostic(void) {
    ...
    CHECK(close(fd) == 0);
    CHECK(sqlite3_open(path, &raw) == SQLITE_OK);
    CHECK(sqlite3_exec(raw, "PRAGMA user_version=35;", NULL, NULL, NULL) == SQLITE_OK);
    CHECK(sqlite3_close(raw) == SQLITE_OK);
    CHECK(trainlog_database_open_with_diagnostic(path, &database, diagnostic, sizeof(diagnostic)) ==
          TRAINLOG_STATUS_SCHEMA_UNSUPPORTED);
    CHECK(database == NULL);
    CHECK(strstr(diagnostic, "schema version 35 is newer") != NULL);
