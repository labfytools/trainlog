...
class BrowserSyncTest(unittest.TestCase):
    def test_button_starts_worker_reconnects_and_displays_committed_draft(self):
        ...
        with tempfile.TemporaryDirectory(prefix="trainlog-web-sync-") as directory:
            ...
            android_env.update(
                {
                    ...
                    "TRAINLOG_SYNC_TRANSPORT_ROOT": str(remote),
                    "TRAINLOG_SYNC_TEST_RELEASE_FILE": str(release),
                }
            )
            android = subprocess.Popen(
                [
                    "./gradlew",
                    "--no-daemon",
                    "testDebugUnitTest",
                    "--tests",
                    "com.labfytools.trainlog.data.SyncDeploymentConversationTest",
                    "--rerun-tasks",
                ],
                cwd=ROOT / "android",
                env=android_env,
            ...
            try:
                peer_file = await_condition(
                    lambda: (
                        (remote / "android-peer-v1.json")
                        if (remote / "android-peer-v1.json").is_file()
                        else None
                    ),
                    120,
                )
                ...
                if android.poll() is None:
                    os.killpg(android.pid, signal.SIGTERM)
                android.wait(timeout=10)
                android_log.close()
                raise AssertionError(
                    f"Android bridge did not advertise:\n{(root/'android.log').read_text(errors='replace')}"
                ) from error
            ...
            env.update(
                {
                    ...
                    "TRAINLOG_SYNC_GENERATION_CONFIG": str(config),
                }
            )
            if mode == "mtp":
                env.update(
            ...
            try:
                ...
                observed = await_condition(
                    lambda: (
                        ...
                        and json.loads(state_path.read_text()).get("phase")
                        in ("local_import_committed", "failed", "interrupted")
                        else None
                    ),
                    70,
                )
                ...
                final = await_condition(
                    lambda: (
                        json.loads(state_path.read_text())
                        if json.loads(state_path.read_text()).get("phase")
                ...
                summary = driver.find_element(
                    By.XPATH, "//summary[contains(.,'Synchronisation confirmée')]"
                )
                summary.click()
                wait.until(
